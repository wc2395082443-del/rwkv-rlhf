...
OUT=/root/autodl-tmp/deepmath_standard_grpo_neg1_lr2e7_200_20260715
...
export TORCH_EXTENSIONS_DIR=/root/autodl-tmp/torch_extensions_deepmath_standard_grpo
...
cp train_rl_standard_grpo.py "$OUT/train_rl_standard_grpo.py"
cp /root/autodl-tmp/run_deepmath_standard_grpo_neg1_lr2e7_200_20260715.sh "$OUT/run.sh"
echo "[$(date '+%F %T')] START standard_grpo neg_adv_weight=1.0 lr=2e-7" | tee "$OUT/status.log"

/root/miniconda3/bin/python train_rl_standard_grpo.py \
  --load_model "$MODEL" \
  --proj_dir "$OUT" \
  --tokenizer "$TOKENIZER" \
  ...
  --grad_cp 1 --total_steps 200 --random_seed 42 \
  ...
  --lr 2e-7 --grad_clip 1.0 --neg_adv_weight 1.0 \
  ...
  --preeval_sample_ratio 1.0 --skip_preeval 0 --skip_posteval 1 --final_full_eval 0 \
