...
class GRPOTrainer:
    ...
    def _opd_generate_teacher_targets(self, problems: List[str]) -> List[Tuple[str, str]]:
        if (not self.cfg.opd_enabled) or self.teacher_model is None or self.teacher_tokenizer is None:
            return []
    ...
    def train_step(self, step: int) -> Dict[str, Any]:
        ...
        opd_pairs = []
        opd_opt = {'opd_loss': 0.0, 'opd_batches': 0}
        if self.cfg.opd_enabled and self.teacher_model is not None and self.teacher_tokenizer is not None:
            opd_pairs = self._opd_generate_teacher_targets([g['problem'] for g in group_infos[:self.cfg.num_questions]])
            opd_opt = self._optimize_opd_sft(opd_pairs)
        _sync_cuda()
        ...
        opd_loss = float(opd_opt.get('opd_loss', 0.0))
        ...
        metrics = {
            ...
            "opd_pairs": len(opd_pairs),
            "opd_batches": int(opd_opt.get("opd_batches", 0)),
            "opd_loss": opd_loss,
            ...
            "opt_zero_grad_time": float(normal_opt.get('phase_times', {}).get('zero_grad_time', 0.0) + extra_opt.get('phase_times', {}).get('zero_grad_time', 0.0)),
            "opt_pad_time": float(normal_opt.get('phase_times', {}).get('pad_time', 0.0) + extra_opt.get('phase_times', {}).get('pad_time', 0.0)),
            "opt_policy_forward_time": float(normal_opt.get('phase_times', {}).get('policy_forward_time', 0.0) + extra_opt.get('phase_times', {}).get('policy_forward_time', 0.0)),
            ...
            "opt_loss_time": float(normal_opt.get('phase_times', {}).get('loss_time', 0.0) + extra_opt.get('phase_times', {}).get('loss_time', 0.0)),
            "opt_backward_time": float(normal_opt.get('phase_times', {}).get('backward_time', 0.0) + extra_opt.get('phase_times', {}).get('backward_time', 0.0)),
            "opt_grad_clip_time": float(normal_opt.get('phase_times', {}).get('grad_clip_time', 0.0) + extra_opt.get('phase_times', {}).get('grad_clip_time', 0.0)),
