...
def parse_args():
    ...
    ap.add_argument("--teacher_model", type=str, default="", help="HF teacher model path for OPD")
...
def main():
    ...
    if str(getattr(args, "teacher_model", "")).strip() and float(args.opd_weight) > 0:
        print(f"??OPD????: {args.teacher_model}")
        teacher_model, teacher_tokenizer = load_hf_teacher(str(args.teacher_model), device=device, dtype=str(args.model_dtype))
