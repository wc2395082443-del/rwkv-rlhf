...
def parse_args():
    ...
    ap.add_argument('--policy_objective', type=str, default='grpo', choices=['grpo', 'real'], help='policy objective')
    ap.add_argument('--real_tau', type=float, default=0.5, help='REAL temperature')
    ap.add_argument('--real_label_source', type=str, default='reward', choices=['reward', 'correct'], help='REAL label source')
    ap.add_argument('--real_reward_weight_cap', type=float, default=2.0, help='cap for inherited reward weight in REAL')
...
def main():
    ...
    cfg = GRPOConfig(
        ...
        policy_objective=str(args.policy_objective),
        real_tau=float(args.real_tau),
        real_label_source=str(args.real_label_source),
        real_reward_weight_cap=float(args.real_reward_weight_cap),
