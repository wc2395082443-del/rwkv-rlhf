...
def calculate_reward(
    ...
    zstd_threshold: float = 2.5,  # 阈值：超过这个压缩率视为复读
...
def calculate_reward_details(
    ...
    format_reward = 1.0 if (is_correct and is_format_correct) else 0.0
    ...
    if text:
        ...
        if len(raw) > 100:
            ...
            if len(comp) > 0:
                ...
                if x > 0:
                    # Exponential + linear penalty. Anchors: x=0.5 -> 0.1, x=1.0 -> 0.25
                    b = 1.0
                    x1, y1 = 0.5, 0.1
                    x2, y2 = 1.0, 0.25
