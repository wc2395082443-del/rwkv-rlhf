...
class GRPOTrainer:
    ...
    def _optimize_trajs(self, trajs: List[Dict[str, Any]], lr_scale: float = 1.0, adv_clip: Optional[float] = None) -> Dict[str, float]:
        ...
        for _ in range(int(self.cfg.ppo_epochs)):
            ...
            for start in range(0, len(trajs), int(self.cfg.micro_batch)):
                batch = trajs[start:start + int(self.cfg.micro_batch)]
                ...
                for bi, traj in enumerate(batch):
                    ...
                    new_logp = logp_all[bi, start_idx:end_idx]
                    ...
                    adv = torch.tensor(float(traj['advantage']), device=self.device, dtype=torch.float32)
                    if adv_clip is not None and adv_clip > 0:
                        adv = torch.clamp(adv, min=-float(adv_clip), max=float(adv_clip))

                    ratio = torch.exp(new_logp - old_logp)
                    ratio_clipped = torch.clamp(ratio, 1.0 - float(self.cfg.clip_eps), 1.0 + float(self.cfg.clip_eps))
                    policy_loss = -torch.min(ratio * adv, ratio_clipped * adv).sum()
                    kl = compute_unbiased_kl(ref_logp, new_logp).sum()
                    ent = curr_entropy.sum()

                    batch_loss = policy_loss if batch_loss is None else batch_loss + policy_loss
                    batch_kl = kl if batch_kl is None else batch_kl + kl
                    batch_entropy = ent if batch_entropy is None else batch_entropy + ent
                    total_tokens += min_len
                    clip_total += ((ratio < (1.0 - float(self.cfg.clip_eps))) | (ratio > (1.0 + float(self.cfg.clip_eps)))).sum().item()
                    clip_total_tokens += ratio.numel()

                if total_tokens <= 0 or batch_loss is None:
                    continue

                normalized_loss = batch_loss / float(total_tokens)
                normalized_kl = batch_kl / float(total_tokens)
                normalized_entropy = batch_entropy / float(total_tokens)
                total_loss = normalized_loss + float(self.cfg.kl_coef) * normalized_kl
                ...
                loss_total += float(normalized_loss.item())
                kl_total += float(normalized_kl.item())
                entropy_total += float(normalized_entropy.item())
                batch_cnt += 1
    ...
    def train_step(self, step: int) -> Dict[str, Any]:
        ...
        sample_count = min(int(self.cfg.num_questions), len(self.train_data))
        sampled_indices = self.rng.sample(range(len(self.train_data)), k=sample_count)
        sampled_questions = [self.train_data[i] for i in sampled_indices]

        prompt_tokens_list = []
        problems = []
        answers = []
        for ex in sampled_questions:
            ...
            answers.append(self._get_answer(ex))
        ...
        comp_tokens_list, old_logps_list, comp_texts_list, truncated_list = self.infer.generate_group_parallel(
            ...
            gen_batch_size=self.cfg.gen_batch_size,
            return_logps=True,
        )
        ...
        for q_idx, (problem, answer, prompt_tokens) in enumerate(zip(problems, answers, prompt_tokens_list)):
            group_trajs = []
            group_rewards = []
            correct_in_group = 0

            for sample_idx in range(int(self.cfg.samples_per_question)):
            ...
            if correct_in_group == 0:
                groups_all_wrong += 1
            elif correct_in_group == len(group_trajs):
            ...
            groups_used += 1
            for traj, reward_val in zip(group_trajs, group_rewards):
                adv = (float(reward_val) - mean_reward) / (std_reward + 1e-8)
                traj['advantage'] = adv
                adv_values.append(adv)
                all_trajs.append(traj)

        opt_stats = self._optimize_trajs(all_trajs, lr_scale=1.0, adv_clip=None)
        ...
        avg_kl = opt_stats['kl_total'] / max(1, opt_stats['batch_cnt'])
        clip_frac = opt_stats['clip_total'] / max(1, opt_stats['clip_total_tokens'])
        ...
        return {
            ...
            'loss': opt_stats['loss_total'],
            'kl': opt_stats['kl_total'],
            'avg_kl': avg_kl,
            'clip_frac': clip_frac,
            ...
            'ts_rms': 0.0,
            'ts_bad': False,
            'avg_entropy': opt_stats['entropy_total'],
            'avg_zstd_penalty': avg_zstd_penalty,
            'avg_zstd_ratio': avg_zstd_ratio,
            'hard_buffer_size': 0,
            'hard_buffer_added': 0,
            ...
            'extra_lr_scale': 0.0,
            'extra_adv_clip': 0.0,
            'dynamic_resample_enabled': 0,
            'dynamic_target_effective_groups': 0,
            'dynamic_effective_groups': groups_used,
            'dynamic_sampling_rounds': 1,
            'dynamic_resample_rounds': 0,
        }

    def train(self, total_steps: int):
        self._log(f"training: total_steps={total_steps}")
        self._log(f"config: num_questions={self.cfg.num_questions}, samples_per_question={self.cfg.samples_per_question}, ppo_epochs={self.cfg.ppo_epochs}, clip_eps={self.cfg.clip_eps}, kl_coef={self.cfg.kl_coef}")
        ...
        for step in range(1, total_steps + 1):
            ...
            if step % self.cfg.save_interval == 0 or step == total_steps:
                ckpt_dir = os.path.join(self.out_dir, f"ckpt_step{step}")
                os.makedirs(ckpt_dir, exist_ok=True)
                try:
                    self.model.save_pretrained(ckpt_dir)
                except Exception:
                    torch.save(self.model.state_dict(), os.path.join(ckpt_dir, "model.pt"))
                torch.save({"step": step, "optimizer": self.opt.state_dict()}, os.path.join(ckpt_dir, "trainer_state.pt"))
