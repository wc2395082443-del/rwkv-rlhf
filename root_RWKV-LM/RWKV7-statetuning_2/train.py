...
def compute_unbiased_kl(ref_logp: torch.Tensor, policy_logp: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        ref_logp: reference model??og???
        policy_logp: ???policy??og???
...
class GRPOTrainer:
    ...
    def __init__(
        ...
        self._zstd = zstd.ZstdCompressor(level=9)
    ...
    def _zstd_stats(self, text: str):
        ...
        try:
            ...
            if not raw:
                return 0, 0, 1.0
            comp = self._zstd.compress(raw)
            return len(raw), len(comp), (len(comp) / len(raw))
        except Exception:
            return 0, 0, 1.0
    ...
    def evaluate(self, step: int, tag: str = "eval", sample_ratio: float = 1.0) -> Optional[float]:
        ...
        for start in range(0, len(data), chunk_size):
            ...
            for i in range(len(prompt_tokens_list)):
                ...
                zstd_raw, zstd_comp, zstd_ratio = self._zstd_stats(comp_text)
                ...
                zstd_raw_bytes += zstd_raw
                zstd_comp_bytes += zstd_comp
                sum_zstd_ratio += zstd_ratio
                zstd_count += 1
        ...
        avg_zstd_ratio = sum_zstd_ratio / max(1, zstd_count)
        ...
        append_jsonl(self.metrics_path, {
            ...
            "zstd_comp_bytes": zstd_comp_bytes,
            "eval_count": total,
            "eval_time": eval_time,
        })
        self._log(
    ...
    def train_step(self, step: int) -> Dict[str, Any]:
        ...
        def _consume_sample(train_idx, prompt_tokens, comp_tokens, old_logps, comp_text, truncated, p_hist):
            ...
            stats["zstd_raw_bytes"] += zstd_raw
            stats["zstd_comp_bytes"] += zstd_comp
            stats["sum_zstd_ratio"] += zstd_ratio
            stats["zstd_count"] += 1
        ...
        # 4b. difficulty weighting (1/p) normalized within buffer
        if all_trajs:
            weights = [1.0 / max(1e-6, t.get("p_hist", 1.0)) for t in all_trajs]
            mean_w = sum(weights) / len(weights) if weights else 1.0
            mean_w = max(1e-6, mean_w)
            for t, w in zip(all_trajs, weights):
                t["advantage"] = max(0.0, t["reward"]) * (w / mean_w)

        # 4c. top-16 filter by gradient proxy (adv * NLL)
        if all_trajs:
            self.model.eval()
            nll_list = []
            with torch.no_grad():
                for start in range(0, len(all_trajs), self.cfg.micro_batch):
                    batch = all_trajs[start:start + self.cfg.micro_batch]
                    ...
                    logp = (logit_tgt - logsumexp).float()
                    del logits
                    for bi, traj in enumerate(batch):
                        ...
                        lp = logp[bi, start_idx:end_idx]
                        if lp.numel() == 0:
                            nll = 0.0
                        else:
                            nll = float((-lp).sum().item())
                        nll_list.append(nll)
            for t, nll in zip(all_trajs, nll_list):
                t["grad_score"] = t.get("advantage", 0.0) * nll
            all_trajs = sorted(all_trajs, key=lambda x: x.get("grad_score", 0.0), reverse=True)[:16]

        adv_values = [t.get("advantage", 0.0) for t in all_trajs]

        # 5. TRAIN
        effective_groups = max(1, groups_used)
        # --- [???] 1. ??????????? Effective Groups ?? Token ??? ---
        # ???????? Global Token Level Normalization
        # ?????oss = Sum(All_Loss) / Sum(All_Tokens)
        # ?? Mini-batch ???????????????Batch_Loss / Global_Total_Tokens
        global_valid_tokens = sum(len(traj["comp_tokens"]) for traj in all_trajs)
        global_valid_tokens = max(1, global_valid_tokens)  # ?????0?????
        loss_total = 0.0
        kl_total = 0.0
        grad_norm = 0.0
        batch_cnt = 0
        ...
        avg_zstd_ratio = stats["sum_zstd_ratio"] / max(1, stats["zstd_count"])
        if adv_values:
        ...
        metrics = {
            ...
            "avg_length_lambda": avg_length_lambda,
            "avg_zstd_ratio": avg_zstd_ratio,
            "zstd_raw_bytes": stats["zstd_raw_bytes"],
            "zstd_comp_bytes": stats["zstd_comp_bytes"],
            "adv_mean": adv_mean,
            "adv_std": adv_std,
