...
class RWKV_Tmix_x070(nn.Module):
    ...
    def forward(self, x, v_first, attention_mask=None):
        ...
        xr, xw, xk, xv, xa, xg = self.torch_addcmul(x, xx)
        ...
        x = self.ln_x(x.reshape(B * T, C).to(self.ln_x.weight.dtype)).view(B, T, C)
...
class RWKV_Tmix_x070_State(RWKV_Tmix_x070):
    ...
    def forward(self, x, v_first, attention_mask=None):
        ...
        x = self.ln_x(x.view(B * T, C).to(self.ln_x.weight.dtype)).view(B, T, C)
...
class RWKV7(nn.Module):
    ...
    def forward_hidden(self, input_ids, inputs_embeds=None, attention_mask=None, **kwargs):
        args = self.args
        B, T = input_ids.size()
    ...
    def forward_normal(self, input_ids, inputs_embeds=None, attention_mask=None, **kwargs):
        x = self.forward_hidden(input_ids, inputs_embeds=inputs_embeds, attention_mask=attention_mask, **kwargs)
        x = self.project_logits(x)
        return x
