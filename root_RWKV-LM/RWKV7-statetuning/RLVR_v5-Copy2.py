        ...
        for t in range(max_new_tokens):
            ...
            if (stop_on_think_close or stop_on_user or stop_on_ngram) and (t % max(1, stop_check_every) == 0):
                for i in range(B):
                    ...
                    stop_hit = False
                    if stop_on_ngram and _has_repeated_ngram(comp_tokens[i], stop_ngram_n, stop_ngram_repeat):
                        stop_hit = True
                    elif _has_repeated_ngram_any(comp_tokens[i], stop_repeat_ngram_n, stop_repeat_ngram_repeat):
                        stop_hit = True
                    elif _has_dominant_token(comp_tokens[i], stop_token_dominance, stop_token_dominance_min):
                        stop_hit = True
                    else:
                        ...
                        if ANSWER_EARLY_STOP_RE.search(s):
                            stop_hit = True
