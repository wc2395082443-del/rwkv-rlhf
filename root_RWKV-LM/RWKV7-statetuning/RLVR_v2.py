...
class DAPOStateTuningTrainer:
    ...
    def evaluate(self, step: int):
        ...
        ex_list = [self.data[i] for i in idxs]
    ...
    def train(self, total_steps: int):
        ...
        for step in range(1, total_steps + 1):
            ...
            while len(buffer) < self.cfg.batch_prompts and tries < self.cfg.dynamic_sampling_max_tries:
                print(f"Collecting: {len(buffer)}/{self.cfg.batch_prompts}...", flush=True)
                tries += 1
                ...
                for pi in range(chunk):
                    ...
                    for gi in range(self.cfg.group_size):
                        ...
                        individual_response = {
                            ...
                            "response": ctext,
                            "reward": float(r),
                            "is_truncated": bool(truncated_flat[idx]),
                            "judge_detail": jdbg
                        }
                        
                        # 定义保存路径，例如在 out_dir 下的 responses.jsonl
...
def main():
    ...
    ap.add_argument("--stop_on_user", action="store_true")
    ap.add_argument("--stop_on_boxed", action="store_true")
    ap.add_argument("--stop_check_every", type=int, default=32)
    ...
    try:
        ...
        print(f"\n[RLVR_v2] Initialized. Training on {len(data)} samples.\n")
