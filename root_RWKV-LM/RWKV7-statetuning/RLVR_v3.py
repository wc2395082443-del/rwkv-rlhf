...
class DAPOStateTuningTrainer:
    def __init__(
        ...
        self._answer_buffer: Dict[int, List[Dict[str, Any]]] = {}
        ...
        self.buffer_save_path = cfg.buffer_save_path
        self.buffer_load_path = cfg.buffer_load_path
        self.buffer_save_interval = max(1, int(cfg.buffer_save_interval))
        self.buffer_cold_path = cfg.buffer_cold_path
        self._cold_start_saved = bool(self.buffer_load_path)
        if self.buffer_load_path:
            self._load_answer_buffer(self.buffer_load_path)
    ...
    def _buffer_items(self) -> int:
        return sum(len(v) for v in self._answer_buffer.values())

    def warmup_buffer(self):
        min_items = int(getattr(self.cfg, "buffer_min_init", 0) or 0)
        if min_items <= 0:
            return
        buffer_items = self._buffer_items()
        if buffer_items >= min_items:
            self._log(f"[buffer] warmup skip: items={buffer_items} >= min_init={min_items}")
            return
        max_rounds = max(1, int(getattr(self.cfg, "buffer_warmup_rounds", 1)))
        self._log(f"[buffer] warmup start: items={buffer_items} target={min_items} rounds={max_rounds}")
        rounds = 0
        while buffer_items < min_items and rounds < max_rounds:
            rounds += 1
            rollout_n = int(self.cfg.rollout_n)
            rollout_idxs = [self.rng.randrange(len(self.train_data)) for _ in range(rollout_n)]
            ...
            for pi, data_idx in enumerate(rollout_idxs):
                ...
                if trunc:
                    ...
                    r, _ = judge_answer_dispatch(
                        judge_type=self.cfg.judge_type,
                        model_response=ctext,
                        gold_answer=gts[pi],
                        llm_judge_fn=judge_answer_llm if self.cfg.judge_type == "llm" else None,
                    )
                if r >= 0.5 and (not trunc):
                    self._update_answer_buffer(data_idx, ctoks, ologp, ctext)

            buffer_items = self._buffer_items()

        self._log(f"[buffer] warmup done: items={buffer_items} rounds={rounds}")
    ...
    def train(self, total_steps: int):
        ...
        for step in range(1, total_steps + 1):
            ...
            buffer_items = sum(len(v) for v in self._answer_buffer.values())
            rollout_rounds = 0
            max_rollout_rounds = max(1, int(self.cfg.dynamic_sampling_max_tries))

            while rollout_rounds < max_rollout_rounds and (rollout_rounds < 1 or buffer_items < train_bsz):
                rollout_rounds += 1
                rollout_idxs = [self.rng.randrange(len(self.train_data)) for _ in range(rollout_n)]
                ex_list = [self.train_data[i] for i in rollout_idxs]
                prompt_strs = [build_prompt(ex.get("problem", "")) for ex in ex_list]
                ...
                for pi, data_idx in enumerate(rollout_idxs):
                    ...
                    if trunc:
                        ...
                        jdbg = {
                            "pred_extracted": None,
                            "gt": normalize_answer(extract_answer(gts[pi]) or gts[pi]),
                            "method": "truncated_skip_judge",
                            "error": None,
                            "pred_parsed": None,
                            "gt_parsed": None,
                    ...
                    append_jsonl(self.gen_dump_path, {
                        ...
                        "group_size": 1,
                        "max_new_tokens": self.cfg.max_new_tokens,
                        "sample": {"text": ctext, "truncated": trunc, "reward": float(r), "judge": jdbg},
                    })
            ...
            if buffer_items < train_bsz:
                self._log("WARN: buffer_items < train_bsz after rollouts; increase rollout_n or max tries.")
                continue

            buffer_keys = [k for k, v in self._answer_buffer.items() if v]
            if not buffer_keys:
                self._log("WARN: empty buffer after rollouts; increase rollout_n or run more steps.")
                continue

            if (not self._cold_start_saved) and self.buffer_cold_path:
                self._save_answer_buffer(self.buffer_cold_path)
                self._cold_start_saved = True
                self._log(f"[buffer] cold start saved to {self.buffer_cold_path}")

            trajs = []
            for _ in range(train_bsz):
                data_idx = self.rng.choice(buffer_keys)
                buf = self._answer_buffer.get(data_idx)
                entry = self._sample_from_buffer(buf)
                ...
                full = prompt_tokens + entry["tokens"]
                trajs.append({
                    "full_tokens": full,
                    "prompt_len": len(prompt_tokens),
                    "comp_len": len(entry["tokens"]),
                    "old_logps": entry["old_logps"],
                    "adv": 1.0,
                    "reward": 1.0,
                })

            train_tries = train_bsz
            ...
            if step % self.cfg.log_interval == 0:
                buffer_items = sum(len(v) for v in self._answer_buffer.values())
                buffer_q = sum(1 for v in self._answer_buffer.values() if v)
                self._log(
                    f"[step {step}/{total_steps}] "
                    f"rollout_n={rollout_n} rollouts={rollout_rounds} train_bsz={train_bsz} train_tries={train_tries} | "
                    f"samples={sample_cnt} acc={sample_correct/max(1,sample_cnt):.3f} trunc={sample_trunc/max(1,sample_cnt):.3f} | "
                    f"buffer_q={buffer_q} buffer_items={buffer_items} train_samples={len(trajs)} | "
            ...
            if self.buffer_save_path and (step % self.buffer_save_interval == 0 or step == total_steps):
                self._save_answer_buffer(self.buffer_save_path)
