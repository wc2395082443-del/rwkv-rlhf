...
LENGTH_REWARD_MIN_LEN = 200

FORCE_ANSWER_SUFFIX = " so the answer is :"
FORCE_ANSWER_SUFFIX_MAX_NEW = 64
...
def _extract_candidate(raw: str):
    if raw is None:
        return None
    raw = _strip_math_delims(str(raw)).strip()
    if not raw:
        return None
    frac = re.findall(r'-?\d+\s*/\s*-?\d+', raw.replace(',', ''))
    if frac:
...
ANSWER_KEYWORD_NUM_RE = re.compile(
    r"(?i)\banswer(?:\s+is)?\b[^A-Za-z0-9]*([-+]?\d+\.?\d*(?:\s*/\s*-?\d+\.?\d*)?)"
)

def _extract_answer_candidates(text: str):
    ...
    while True:
        ...
        if got is not None:
            inner, end_idx = got
            cand = _extract_candidate(inner)
            if cand:
                candidates.append((pos, cand))
            idx = end_idx + 1
        else:
            idx = pos + len(key)

    # 2) answer is/answer: ${answer} (no letters between keyword and number)
    for m in ANSWER_KEYWORD_NUM_RE.finditer(text):
        cand = _extract_candidate(m.group(1))
        if cand:
            candidates.append((m.start(1), cand))

    # 4) ${answer} (standalone line)
    for m in re.finditer(r'(?m)^\s*([-+]?\d+\s*/\s*[-+]?\d+|[-+]?\d+\.?\d*)\s*$', text):
        cand = _extract_candidate(m.group(1))
        if cand:
...
def _extract_answer_candidates_with_pos(text: str):
    if not text or not isinstance(text, str):
        return []
    ...
    while True:
        ...
        if got is not None:
            ...
            cand = _extract_candidate(inner)
            if cand:
                candidates.append((pos, end_idx + 1, cand))
            idx = end_idx + 1
        else:
            idx = pos + len(key)
    ...
    for m in ANSWER_KEYWORD_NUM_RE.finditer(text):
        cand = _extract_candidate(m.group(1))
        if cand:
            candidates.append((m.start(1), m.end(1), cand))
...
def _extract_explicit_answer_candidates_with_pos(text: str):
    if not text or not isinstance(text, str):
        return []
...
def _token_index_at_char(tokens, decode_fn, char_end: int):
    if tokens is None or decode_fn is None or char_end is None:
        return None
    total = 0
    for i, tok in enumerate(tokens):
        try:
            total += len(decode_fn([tok]))
        except Exception:
            total += 0
        if total >= char_end:
            return i + 1
...
def _first_correct_answer_token_idx(pred_text: str, gold_text: str, response_tokens, decode_fn):
    candidates = _extract_explicit_answer_candidates_with_pos(pred_text)
    if not candidates:
        return None
    for _start, end_pos, cand in candidates:
        if compare_answers(cand, gold_text):
            return _token_index_at_char(response_tokens, decode_fn, end_pos)
    return None


def extract_answer(text):
    """Extract answer from model output"""
    answers = _extract_answer_candidates(text)
    if not answers:
        return None
    return answers[-1]


def extract_all_answer(text):
    """Extract all candidate answers from model output"""
    return _extract_answer_candidates(text)
...
def _compute_reward_components(correct_any: bool, correct_boxed: bool, has_boxed: bool, truncated: bool, repeat_penalty: bool) -> Tuple[float, Dict[str, float]]:
    reward = 0.0
    if correct_any:
        reward += 1.0
    ...
    parts = {
        "correct_any": 1.0 if correct_any else 0.0,
        "boxed_present": 0.0,
        "boxed_correct": 1.0 if (correct_boxed and not truncated) else 0.0,
        "truncated_penalty": 0.0,
        "repeat_penalty": 0.0,
...
def _apply_repeat_reward_penalty(reward: float, correct_any: bool, repeat_penalty: bool, parts: Dict[str, float]) -> float:
    ...
    if correct_any:
        ...
        reward = -0.5
    parts["repeat_penalty"] = reward - before
    return reward
...
def judge_answer_dispatch(
    ...
    min_len: int = LENGTH_REWARD_MIN_LEN,
    ...
    all_answers = extract_all_answer(pred_text)
    multi_answer_norms = []
    for ans in all_answers:
        norm = normalize_answer(ans)
        if norm is None or norm == "":
            continue
        multi_answer_norms.append(norm)
    multi_answer_count = len(set(multi_answer_norms))
    multi_answer_penalty = 0.0
    repeat_penalty = _has_repeated_ngrams(pred_text, n=16, repeat=6)
    gold_extracted = extract_answer(gold_answer) or gold_answer
    first_correct_token_idx = _first_correct_answer_token_idx(pred_text, gold_extracted, response_tokens, decode_fn)
    ...
    if judge_type == "llm":
        ...
        gold_norm = normalize_answer(gold_extracted)
        reward, parts = _compute_reward_components(correct_any, correct_boxed, has_boxed, truncated, repeat_penalty)
        length_lambda = 0.0
        length_reward = 0.0
        if response_len is not None and max_len is not None and max_len > min_len:
            length_lambda = 0.5 - (float(response_len) - float(min_len)) / float(max_len - min_len)
            if correct_any:
                length_reward = length_lambda * LENGTH_REWARD_WEIGHT
            else:
                length_reward = min(0.0, length_lambda) * LENGTH_REWARD_WEIGHT
            reward += length_reward
        parts["length_lambda"] = length_lambda
        parts["length_reward"] = length_reward
        reward = _apply_repeat_reward_penalty(reward, correct_any, repeat_penalty, parts)
        post_answer_penalty = 0.0
        post_answer_tokens = None
        if first_correct_token_idx is not None and response_len is not None and max_len is not None and max_len > 0:
            post_answer_tokens = max(0, int(response_len) - int(first_correct_token_idx))
            post_answer_penalty = -float(post_answer_tokens) * (3.0 / float(max_len))
            reward += post_answer_penalty
        parts["post_answer_penalty"] = post_answer_penalty
        parts["post_answer_tokens"] = post_answer_tokens
        parts["first_correct_pos"] = first_correct_token_idx
        parts["multi_answer_penalty"] = multi_answer_penalty
        parts["multi_answer_count"] = multi_answer_count
        dbg = {
...
ANSWER_EARLY_STOP_RE = re.compile(r"(?i)(?:%s)\s*[:=\-]*\s*(\\boxed\{[^}]+\}|-?\d+\s*/\s*-?\d+|-?\d+\.?\d*)" % "|".join(re.escape(k) for k in ANSWER_KEYWORDS))

def _has_repeated_ngram_any(tokens, n: int, repeat: int) -> bool:
    if n <= 0 or repeat <= 1:
        return False
    total = n * repeat
    if len(tokens) < total:
...
def _has_dominant_token(tokens, ratio: float, min_tokens: int) -> bool:
    if ratio <= 0 or len(tokens) < max(1, int(min_tokens)):
        return False
    counts = {}
    for t in tokens:
        counts[t] = counts.get(t, 0) + 1
    top = max(counts.values())
    return (top / float(len(tokens))) >= float(ratio)


def _has_repeated_ngrams(text: str, n: int = 16, repeat: int = 3) -> bool:
...
def _has_repeated_ngram(tokens, n, repeat):
    if n <= 0 or repeat <= 1:
        return False
    total = n * repeat
    if len(tokens) < total:
...
class DAPOConfig:
    ...
    stop_on_ngram: bool = False
    stop_ngram_n: int = 16
    stop_ngram_repeat: int = 5
    stop_repeat_ngram_n: int = 16
    stop_repeat_ngram_repeat: int = 3
    stop_token_dominance: float = 0.0
    stop_token_dominance_min: int = 0
...
class AlbatrossBatchInference:
    ...
    def generate_group_parallel(
        ...
        stop_on_ngram: bool,
        stop_ngram_n: int,
        stop_ngram_repeat: int,
        stop_repeat_ngram_n: int,
        stop_repeat_ngram_repeat: int,
        stop_token_dominance: float,
        stop_token_dominance_min: int,
        ...
        # 3. 主生成循环
        forced_tail = self.encode(FORCE_ANSWER_SUFFIX) if FORCE_ANSWER_SUFFIX else []
        tail_inserted = False
        forced_queue: List[int] = []
        extra_budget = FORCE_ANSWER_SUFFIX_MAX_NEW if FORCE_ANSWER_SUFFIX else 0
        total_steps = max_new_tokens + len(forced_tail) + extra_budget

        for t in range(total_steps):
            if not active.any():
                break

            if (not tail_inserted) and t >= max_new_tokens:
                if active.any() and forced_tail:
                    forced_queue = list(forced_tail)
                tail_inserted = True
            ...
            if forced_queue:
                forced_token = True
                forced_id = forced_queue.pop(0)
                token_ids = torch.full((B,), int(forced_id), device=last_logits.device, dtype=torch.long)
                ...
                # --- C. Temperature 缩放 (Logits 上，先缩放再过滤) ---
            ...
            # --- J. 停止准则检查 ---
            if (stop_on_think_close or stop_on_user or stop_on_ngram) and (t % max(1, stop_check_every) == 0):
                for i in range(B):
                    if not active[i]:
                        continue
                    ...
                    stop_hit = False
                    if stop_on_ngram:
                        if _has_repeated_ngram_any(w, stop_ngram_n, stop_ngram_repeat):
                            stop_hit = True
                        elif _has_repeated_ngram(w, stop_repeat_ngram_n, stop_repeat_ngram_repeat):
                            stop_hit = True
                        elif _has_dominant_token(w, stop_token_dominance, stop_token_dominance_min):
                            stop_hit = True

                    if (not stop_hit) and (stop_on_think_close or stop_on_user):
...
class DAPOStateTuningTrainer:
    def __init__(
        ...
        self.eval_subset_data = []
        if self.test_data:
            # Deterministic 1/5 subset for faster evals (no randomness).
            self.eval_subset_data = [ex for i, ex in enumerate(self.test_data) if (i % 5) == 0]
    ...
    def _posterior_rate(self, idx: int) -> float:
        k = float(self._prior_k)
        mean = float(self._prior_mean)
        num = float(self._correct_cnt[idx]) + mean * k
        den = float(self._attempt_cnt[idx]) + k
        if den <= 0:
            return mean
        p = num / den
        if p < 0.0:
    ...
    def _sample_problem_batch(self, exclude: set, n: int) -> list:
        candidates = [i for i in range(len(self.train_data)) if i not in exclude]
        if not candidates:
            return [self.rng.randrange(len(self.train_data)) for _ in range(int(n))]
        weights = [self._problem_weight(self._posterior_rate(i)) for i in candidates]
        k = min(int(n), len(candidates))
        chosen = []
        seen = set()
        while len(chosen) < k:
            picks = self.rng.choices(candidates, weights=weights, k=k - len(chosen))
            for idx in picks:
                if idx in seen:
                    continue
                seen.add(idx)
    ...
    def _infer_once(self, problem: str, gt: str, max_new: int, temperature: float, top_p: float, top_k: int) -> Dict[str, Any]:
        ...
        comp_tokens, _, comp_texts, truncs = self.infer.generate_group_parallel(
            ...
            stop_on_ngram=self.cfg.stop_on_ngram,
            stop_ngram_n=self.cfg.stop_ngram_n,
            stop_ngram_repeat=self.cfg.stop_ngram_repeat,
            stop_repeat_ngram_n=self.cfg.stop_repeat_ngram_n,
            stop_repeat_ngram_repeat=self.cfg.stop_repeat_ngram_repeat,
            stop_token_dominance=self.cfg.stop_token_dominance,
            stop_token_dominance_min=self.cfg.stop_token_dominance_min,
        ...
        r, jdbg = judge_answer_dispatch(
            ...
            response_len=len(comp_tokens[0]),
            max_len=self.cfg.max_new_tokens,
            response_tokens=comp_tokens[0],
    ...
    def _select_eval_data(self, step: int):
        if step == 200:
            return self.test_data if self.test_data else self.train_data
        if step == 0:
            if self.eval_subset_data:
                return self.eval_subset_data
            return self.test_data if self.test_data else self.train_data
        if self.eval_subset_data:
            return self.eval_subset_data
        return self.test_data if self.test_data else self.train_data
    ...
    def evaluate(self, step: int, dataset=None, tag: str = "eval"):
        if dataset is None:
            eval_data = self._select_eval_data(step)
        else:
            eval_data = dataset
        if not eval_data:
        ...
        for start in range(0, len(eval_data), chunk_size):
            ...
            for i in range(len(prompt_tokens_list)):
                ...
                r, jdbg = judge_answer_dispatch(
                    ...
                    truncated=trunc,
                    llm_judge_fn=judge_answer_llm if self.cfg.judge_type == "llm" else None,
                    response_len=len(comp_tokens_flat[i]),
                    max_len=self.cfg.max_new_tokens,
                )
    ...
    def train(self, total_steps: int):
        self._log(f"train begin: steps={total_steps} rollout_samples={self.cfg.rollout_samples} num_responses={self.cfg.num_responses} "
        ...
        self._prior_k = float(total_steps) * float(self.cfg.rollout_samples) * float(self.cfg.num_responses) / max(1, len(self.train_data))
        self._log(f"[prior] mean={self._prior_mean:.2f} k={self._prior_k:.3f}")
        ...
        for step in range(1, total_steps + 1):
            t0 = time.time()
            rollout_samples = int(self.cfg.rollout_samples)
            num_responses = int(self.cfg.num_responses)
            ...
            if not self.train_data:
                self._log("WARN: empty train data.")
                continue

            if len(self.train_data) >= rollout_samples:
                rollout_idxs = self.rng.sample(range(len(self.train_data)), rollout_samples)
            else:
                rollout_idxs = [self.rng.randrange(len(self.train_data)) for _ in range(rollout_samples)]

            ex_list = [self.train_data[i] for i in rollout_idxs]
            ...
            for gi, data_idx in enumerate(rollout_idxs):
                group_samples = []
                group_rewards = []
                group_correct = []

                for ri in range(num_responses):
                    flat_idx = gi * num_responses + ri
                    if flat_idx >= len(comp_tokens_flat):
                        continue

                    ctoks = comp_tokens_flat[flat_idx]
                    ologp = old_logps_flat[flat_idx]
                    ctext = comp_text_flat[flat_idx]
                    trunc = bool(truncated_flat[flat_idx])
                    ...
                    r, jdbg = judge_answer_dispatch(
                        ...
                        truncated=trunc,
                        llm_judge_fn=judge_answer_llm if self.cfg.judge_type == "llm" else None,
                        response_len=len(ctoks),
                        max_len=self.cfg.max_new_tokens,
                    )

                    is_correct = bool(jdbg.get("correct"))
                    if is_correct:
                        sample_correct += 1
                    if trunc:
                        sample_trunc += 1

                    sample_reward_total += float(r)

                    self._update_stats(data_idx, is_correct)
                    ...
                    individual_response = {
                        ...
                        "prompt_idx": gi,
                        "group_idx": ri,
                        "problem": ex_list[gi].get("problem", ""),
                        "response": ctext,
                        "reward": float(r),
                        "is_truncated": trunc,
                        "judge_detail": jdbg,
                    ...
                    full = prompt_tokens_list[gi] + ctoks
                    group_samples.append({
                        "full_tokens": full,
                        "prompt_len": len(prompt_tokens_list[gi]),
                        "comp_len": len(ologp),
                        "old_logps": ologp,
                        "reward": float(r),
                    })
                    group_rewards.append(float(r))
                    group_correct.append(1 if is_correct else 0)

                if not group_samples:
                    continue

                correct_sum = sum(group_correct)
                if correct_sum == 0 or correct_sum == len(group_correct):
                    groups_skipped += 1
                    continue

                groups_used += 1
                used_indices.append(int(data_idx))

                g_mean = sum(group_rewards) / float(len(group_rewards))
                g_var = sum((x - g_mean) ** 2 for x in group_rewards) / float(len(group_rewards))
                g_std = math.sqrt(g_var)
                if g_std < 1e-6:
                    g_std = 1.0

                for tr in group_samples:
                    adv = (float(tr["reward"]) - g_mean) / g_std
                    if adv < 0:
                        adv = adv * float(self.cfg.neg_w)
                    tr["adv"] = adv
                    trajs.append(tr)

            self._write_train_ids(step, used_indices, attempts)
            ...
            avg_r = sample_reward_total / max(1, sample_cnt)
            dt = time.time() - t0
            ...
            if step % self.cfg.log_interval == 0:
                ...
                self._log(
                    ...
                    f"rollout_samples={rollout_samples} num_responses={num_responses} groups_used={groups_used} groups_skipped={groups_skipped} attempts={attempts} | "
            ...
            if step % self.cfg.save_interval == 0 or step == total_steps:
                ...
                full_data = self.test_data if self.test_data else self.train_data
                if full_data:
                    self.model.eval()
                    self.evaluate(step=step, dataset=full_data, tag="full_ckpt")

                # plot metrics after each checkpoint
                try:
                    ...
                    self._log(f"[plot_metrics] failed at step {step}: {e!r}")
...
def main():
    ...
    ap.add_argument("--eval_json", type=str, default=None, help="optional eval jsonl")
    ...
    ap.add_argument("--rollout_samples", type=int, default=24)
    ap.add_argument("--num_responses", type=int, default=8)
    ...
    ap.add_argument("--neg_w", type=float, default=1.0)
    ...
    ap.add_argument("--time_state_l2", type=float, default=1e-7)
    ...
    ap.add_argument("--stop_on_boxed", action="store_true", default=False)
    ...
    ap.add_argument("--stop_on_ngram", action="store_true", default=False)
    ap.add_argument("--no_stop_on_ngram", action="store_false", dest="stop_on_ngram")
    ap.add_argument("--stop_ngram_n", type=int, default=16)
    ap.add_argument("--stop_ngram_repeat", type=int, default=5)
    ap.add_argument("--stop_repeat_ngram_n", type=int, default=16)
    ap.add_argument("--stop_repeat_ngram_repeat", type=int, default=3)
    ap.add_argument("--stop_token_dominance", type=float, default=0.0)
    ap.add_argument("--stop_token_dominance_min", type=int, default=0)
    ...
    # --- 功能实现 1: 选取数据集前 n 条 ---
    print(f"Loading train data from {args.train_jsonl}...")
    ...
    print(f"Train data loaded: {len(train_data)} samples selected.")

    if args.eval_json:
        print(f"Loading eval data from {args.eval_json}...")
        test_data = read_jsonl(args.eval_json)
        if not test_data:
            raise RuntimeError("empty eval_json")
        print(f"Using full train set: train={len(train_data)} eval={len(test_data)}")
        ...
        test_size = min(128, len(train_data) - 1)
        test_idxs = idxs[:test_size]
        train_idxs = idxs[test_size:]
        data_all = train_data
        train_data = [data_all[i] for i in train_idxs]
        test_data = [data_all[i] for i in test_idxs]
    ...
    cfg = DAPOConfig(
        ...
        stop_on_ngram=bool(args.stop_on_ngram),
        stop_ngram_n=int(args.stop_ngram_n),
        stop_ngram_repeat=int(args.stop_ngram_repeat),
    ...
    try:
        ...
        if args.run_full_pre:
            pre_acc = trainer.evaluate(step=0, dataset=test_data, tag="full_pre")
            if pre_acc is not None:
                trainer._prior_mean = max(1e-3, min(0.999, float(pre_acc)))
                print(f"[prior] set prior_mean from pre_eval: {trainer._prior_mean:.4f}")
        else:
            print("[eval] skip full_pre (use --run_full_pre to enable)")
        trainer.evaluate(step=0, tag="eval")
        trainer.train(total_steps=int(args.total_steps))
        trainer.model.eval()
        trainer.evaluate(step=int(args.total_steps), dataset=test_data, tag="full_post")
        ...
        try:
            ...
            plot_script = os.path.join(os.path.dirname(__file__), 'plot_metrics.py')
            out_plot = os.path.join(args.out_dir, 'metrics_plot.png')
            if os.path.isfile(plot_script) and os.path.isfile(metrics_path):
                subprocess.run([sys.executable, plot_script, '--metrics', metrics_path, '--out', out_plot], check=False)
        except Exception as e:
            print(f'[plot_metrics] failed: {e!r}')
