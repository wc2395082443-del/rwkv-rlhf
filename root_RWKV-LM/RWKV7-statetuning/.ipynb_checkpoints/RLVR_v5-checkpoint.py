...
def _compute_reward_components(correct_any: bool, correct_boxed: bool, has_boxed: bool, truncated: bool) -> Tuple[float, Dict[str, float]]:
    ...
    parts = {
        "correct_any": 0.7 if correct_any else 0.0,
        "boxed_present": 0.1 if (has_boxed and not truncated) else 0.0,
        "boxed_correct": 0.2 if (correct_boxed and not truncated) else 0.0,
...
class DAPOStateTuningTrainer:
    def __init__(
        ...
        if self.test_data:
            # Deterministic 1/4 subset for faster evals (no randomness).
            self.eval_subset_data = [ex for i, ex in enumerate(self.test_data) if (i % 4) == 0]
...
def main():
    ...
    try:
        ...
        if args.run_full_pre:
            trainer.evaluate(step=0, dataset=test_data, tag="full_pre")
        else:
