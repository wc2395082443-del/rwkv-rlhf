...
def _compute_reward_components(correct_any: bool, correct_boxed: bool, has_boxed: bool, truncated: bool, repeat_penalty: bool) -> Tuple[float, Dict[str, float]]:
    ...
    parts = {
        ...
        "repeat_penalty": -0.1 if repeat_penalty else 0.0,
...
def judge_answer_dispatch(
    ...
    if judge_type == "llm":
        ...
        length_lambda = 0.0
        length_reward = 0.0
        if response_len is not None and max_len is not None and max_len > min_len and correct_boxed:
...
class DAPOConfig:
    ...
    stop_ngram_n: int = 8
    stop_ngram_repeat: int = 3
    stop_repeat_ngram_n: int = 16
    stop_repeat_ngram_repeat: int = 3
    stop_token_dominance: float = 0.5
    stop_token_dominance_min: int = 64
...
class AlbatrossBatchInference:
    ...
    def generate_group_parallel(
        ...
        for t in range(max_new_tokens):
            ...
            # --- J. 停止准则检查 ---
            if (stop_on_think_close or stop_on_user) and (t % max(1, stop_check_every) == 0):
                for i in range(B):
                    if not active[i]:
                        continue
...
def main():
    ...
    ap.add_argument("--neg_w", type=float, default=0.05)
    ...
    ap.add_argument("--stop_ngram_n", type=int, default=8)
    ap.add_argument("--stop_ngram_repeat", type=int, default=3)
    ap.add_argument("--stop_repeat_ngram_n", type=int, default=16)
    ap.add_argument("--stop_repeat_ngram_repeat", type=int, default=3)
    ap.add_argument("--stop_token_dominance", type=float, default=0.5)
    ap.add_argument("--stop_token_dominance_min", type=int, default=64)
