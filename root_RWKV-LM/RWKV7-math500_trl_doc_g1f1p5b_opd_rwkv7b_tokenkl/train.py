...
class GRPOTrainer:
    ...
    @torch.no_grad()
    def _teacher_forward_logits(self, input_ids: torch.Tensor) -> Optional[torch.Tensor]:
        if not self._teacher_ready():
            return None
        self.teacher_model.eval()
        if self.teacher_kind == "rwkv":
                ...
                for start in range(0, len(trajs_sorted), self.cfg.micro_batch):
                    ...
                    with torch.no_grad():
                        ...
                        if self._teacher_ready():
                            teacher_logits = self._teacher_forward_logits(inp)
                        torch.cuda.empty_cache()
                    ...
                    for bi, traj in enumerate(batch):
                        ...
                        if self._teacher_ready() and teacher_logits is not None:
                            teacher_slice = teacher_logits[bi, start_idx:end_idx, :][:min_len]
                            student_slice = self._last_policy_logits[bi, start_idx:end_idx, :][:min_len]
                            if teacher_slice.size(0) > 0 and student_slice.size(0) > 0:
                                distill_temp = max(1e-5, float(getattr(self.cfg, 'opd_temperature', 1.0)))
                                s_log_probs = F.log_softmax(student_slice.float() / distill_temp, dim=-1)
                                t_probs = F.softmax(teacher_slice.float() / distill_temp, dim=-1)
                                token_kl = F.kl_div(s_log_probs, t_probs, reduction='sum') * (distill_temp * distill_temp) / max(1, min_len)
                                batch_opd_loss += token_kl
                                batch_opd_items += 1
