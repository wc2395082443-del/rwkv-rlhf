...
def parse_args():
    ...
    ap.add_argument("--ga_enable", action="store_true", help="enable GradAlign data selection")
    ap.add_argument("--ga_mode", type=str, default="full", choices=["full", "lite"], help="GradAlign mode")
    ap.add_argument("--ga_val_size", type=int, default=64, help="reserved train examples used as GradAlign anchors")
    ap.add_argument("--ga_selection_interval", type=int, default=10, help="steps between GradAlign curriculum refreshes")
    ap.add_argument("--ga_candidate_pool_size", type=int, default=96, help="candidate pool size per GradAlign round")
    ap.add_argument("--ga_selection_ratio", type=int, default=4, help="keep top 1/q candidates")
    ap.add_argument("--ga_val_rollouts", type=int, default=8, help="rollouts per GradAlign validation problem")
    ap.add_argument("--ga_candidate_rollouts", type=int, default=4, help="rollouts per GradAlign candidate problem")
    ap.add_argument("--ga_candidate_keep_ratio", type=float, default=0.5, help="fraction of all-wrong groups kept by GradAlign-lite")
    ap.add_argument("--ga_score_threshold", type=float, default=-1.0, help="minimum GradAlign cosine score to keep")
    ap.add_argument("--ga_grad_batch_questions", type=int, default=4, help="questions per GradAlign gradient micro-batch")
    ap.add_argument("--ga_max_new_tokens", type=int, default=256, help="max completion tokens used for GradAlign gradient estimation")
    ap.add_argument("--ga_main_buffer_ratio", type=float, default=0.25, help="fraction of normal-step questions mixed in from high-score hard buffer")
    ap.add_argument("--ga_extra_top_ratio", type=float, default=0.5, help="fraction of extra-step hard questions taken from top GradAlign scores")
    ap.add_argument("--ga_hard_target_size", type=int, default=16, help="number of hard-buffer problems used to form the target gradient")
    ap.add_argument("--ga_train_keep_ratio", type=float, default=0.25, help="fraction of random candidate problems kept for actual training")
...
def main():
    ...
    print(f"可训练参数: {trainable} (fp32={fp32_trainable})")
    ...
    cfg = GRPOConfig(
        ...
        ga_enable=bool(args.ga_enable),
        ga_mode=str(args.ga_mode),
        ga_val_size=int(args.ga_val_size),
        ga_selection_interval=int(args.ga_selection_interval),
        ga_candidate_pool_size=int(args.ga_candidate_pool_size),
        ga_selection_ratio=int(args.ga_selection_ratio),
        ga_val_rollouts=int(args.ga_val_rollouts),
        ga_candidate_rollouts=int(args.ga_candidate_rollouts),
        ga_candidate_keep_ratio=float(args.ga_candidate_keep_ratio),
        ga_score_threshold=float(args.ga_score_threshold),
        ga_grad_batch_questions=int(args.ga_grad_batch_questions),
        ga_max_new_tokens=int(args.ga_max_new_tokens),
        ga_main_buffer_ratio=float(args.ga_main_buffer_ratio),
        ga_extra_top_ratio=float(args.ga_extra_top_ratio),
        ga_hard_target_size=int(args.ga_hard_target_size),
        ga_train_keep_ratio=float(args.ga_train_keep_ratio),
    )
