...
class GRPOTrainer:
    def __init__(
        ...
        # Keep only the lightweight buffer-ranking path; the full curriculum path was too costly.
    ...
    def _gradalign_advantages(self, rewards: List[float]) -> List[float]:
        if not rewards:
            return []
    ...
    def _gradalign_build_groups(self, examples: List[Dict[str, Any]], group_size: int, binary_reward: bool = False) -> List[Dict[str, Any]]:
        ...
        for q_idx, ex in enumerate(examples):
            ...
            for i in range(group_size):
                ...
                _, is_correct, _, _ = calculate_reward_details(
                    text=comp_text,
                    ...
                    repeat_ngram=self._has_repeated_ngrams(comp_text, n=16, repeat=5),
    ...
    def _gradalign_group_vector(self, groups: List[Dict[str, Any]]) -> Optional[torch.Tensor]:
        ...
        for group in groups:
            ...
            for traj, adv in zip(group.get("group_trajs", []), advs):
                if abs(adv) <= 1e-6:
                    continue
                comp_tokens = traj.get("comp_tokens", [])
                if not comp_tokens:
                    continue
                comp_tokens = comp_tokens[:max_comp_tokens]
                if not comp_tokens:
                    continue
                seqs, _ = self._pad_batch([prompt_tokens + comp_tokens], pad_id=0)
                ...
                if token_logp.numel() == 0:
                    del seqs, inp, tgt, logits, logp, token_logp
                    continue
                ...
                if flat is not None:
                    ...
                    used_terms += 1
                del seqs, inp, tgt, logits, logp, token_logp, objective, grads, flat
    ...
    def _refresh_gradalign_target(self, step: int, force: bool = False) -> bool:
        if not self._ga_enabled or not self._ga_val_examples:
            return False
    ...
    def _gradalign_refresh_curriculum(self, step: int) -> bool:
        if not self._ga_full_enabled:
            return False
        interval = max(1, int(self.cfg.ga_selection_interval))
        if self._ga_curriculum_indices and (step - self._ga_last_curriculum_step) < interval:
            return False
        refreshed = int(self._refresh_gradalign_target(step, force=True))
        pool_size = max(int(self.cfg.num_questions), int(self.cfg.ga_candidate_pool_size))
        if len(self.train_data) >= pool_size:
            candidate_indices = self.rng.sample(range(len(self.train_data)), pool_size)
        else:
            candidate_indices = [self.rng.randrange(len(self.train_data)) for _ in range(pool_size)]
        examples = [self._make_train_example(i) for i in candidate_indices]
        groups = self._gradalign_build_groups(
            examples,
            max(1, int(self.cfg.ga_candidate_rollouts)),
            binary_reward=False,
        )
        keep_n = max(int(self.cfg.num_questions), int(math.ceil(float(len(groups)) / max(1, int(self.cfg.ga_selection_ratio)))))
        selected, stats = self._gradalign_score_groups(groups, step, keep_n=keep_n, refresh_target=False)
        self._ga_curriculum_indices = [int(group.get("train_idx", -1)) for group in selected if int(group.get("train_idx", -1)) >= 0]
        self._ga_last_curriculum_step = step
        self._ga_last_curriculum_stats = dict(stats)
        self._ga_last_curriculum_stats.update({
            "ga_curriculum_size": len(self._ga_curriculum_indices),
            "ga_pool_size": len(groups),
            "ga_refreshed": refreshed,
        })
        return True
    ...
    def train_step(self, step: int) -> Dict[str, Any]:
        ...
        if self._ga_enabled:
            ...
            if self._ga_target_grad is not None and self._ga_target_norm > 1e-12:
                ...
                selected_groups, _ = self._gradalign_score_groups(candidate_groups, step, keep_n=int(self.cfg.num_questions), refresh_target=False)
        ...
        if len(sampled_indices) < int(self.cfg.num_questions):
            ...
            pool = [i for i in range(len(self.train_data)) if i not in existing]
            if len(pool) >= remain:
        ...
        for q_idx, (question, train_idx) in enumerate(zip(sampled_questions, sampled_indices)):
            group_infos.append({
                "q_idx": q_idx,
                "train_idx": int(train_idx),
                "problem": sampled_examples[q_idx]["problem"],
                "answer": sampled_examples[q_idx]["answer"],
                "prompt_tokens": sampled_examples[q_idx]["prompt_tokens"],
                "group_rewards": [],
                "group_trajs": [],
                "correct_in_group": 0,
                "sampled": 0,
