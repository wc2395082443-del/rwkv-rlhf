...
CUDA_DIR = os.path.join(_THIS_DIR, "cuda")
########################################################################################################
# CUDA Kernel
########################################################################################################
def RUN_CUDA_RWKV7g():
...
if os.environ["WKV"] == 'fla':
    ...
    if 'x070' in os.environ["RWKV_MY_TESTING"]:
        ...
        if os.environ["RWKV_TRAIN_TYPE"] == 'state':
            if os.environ["RWKV_FLOAT_MODE"] == 'bf16':
                flags = ['-res-usage', f'-D_N_={HEAD_SIZE}', f"-D_CHUNK_LEN_={CHUNK_LEN}", "--use_fast_math", "-O3", "-Xptxas", "-O3", "--extra-device-vectorization","--allow-unsupported-compiler"]
                load(name="rwkv7_state_clampw", sources=[
        os.path.join(CUDA_DIR, "rwkv7_state_clampw.cu"),
        os.path.join(CUDA_DIR, "rwkv7_state_clampw.cpp"),
                ...
                flags = ['-res-usage', f'-D_N_={HEAD_SIZE}', "-D_FP32_", f"-D_CHUNK_LEN_={CHUNK_LEN}", "--use_fast_math", "-O3", "-Xptxas", "-O3", "--extra-device-vectorization","--allow-unsupported-compiler"]
                load(name="rwkv7_state_clampw", sources=[
        os.path.join(CUDA_DIR, "rwkv7_state_clampw.cu"),
        os.path.join(CUDA_DIR, "rwkv7_state_clampw.cpp"),
    ], is_python_module=False, verbose=True, extra_cflags=["-D_FP32_"], extra_cuda_cflags=flags)
        ...
        if os.environ["RWKV_TRAIN_TYPE"] == 'infctx':
            wkv6state_cuda = load(name="wkv6infctx", sources=["cuda/wkv6infctx_op.cpp", f"cuda/wkv6infctx_cuda.cu"],
                            verbose=True, extra_cuda_cflags=["-res-usage", "--use_fast_math", "-O3", "-Xptxas", "-O3", "--extra-device-vectorization", f"-D_N_={HEAD_SIZE}", f"-D_T_={int(os.environ['RWKV_CTXLEN'])}","--allow-unsupported-compiler"])
    ...
    extra_cuda_cflags=[
        ...
        '--use_fast_math', 
        '-O3', 
        '-Xptxas', '-O3',  # 分开写，不要写成一个字符串 "-Xptxas -O3"
        '--extra-device-vectorization', 
        f'-D_N_={HEAD_SIZE}',
        '-allow-unsupported-compiler'
    ],
    # 顺便加上之前说的 LNK1181 补丁
    extra_ldflags=['/NODEFAULTLIB:aio.lib', '/NODEFAULTLIB:cufile.lib']
)    
