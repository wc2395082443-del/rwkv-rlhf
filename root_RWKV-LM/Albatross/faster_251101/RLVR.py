...
# 假设 model 是已加载的 RWKV_x070 实例
# optimizer 是针对 model 参数的优化器
...
# 假设你已经定义了 model, tokenizer, 以及之前的 extract_answer 等函数

class GRPO_StateTuning_Trainer:
    def __init__(self, model, tokenizer, lr=1e-3): # State-tuning 的学习率通常可以大一点
        self.model = model
        self.tokenizer = tokenizer
        
        # 确保模型进入 state tuning 模式
        self.model.init_state_tuning()
        
        # 优化器只管理这两个参数
        self.optimizer = torch.optim.Adam([
            self.model.learnable_state_0,
            self.model.learnable_state_1
        ], lr=lr)
        
        self.group_size = 8
        self.beta = 0.05

    def train_step(self, question, gold_answer):
        prompt = build_prompt(question)
        prompt_ids = torch.tensor(self.tokenizer.encode(prompt), device='cuda').unsqueeze(0)
        ...
        with torch.no_grad():
            # 获取可学习状态的副本作为起点
            start_state = self.model.get_trainable_state(bsz=1)
            # 先处理 Prompt
            # 注意：forward 会直接修改 state，所以需要传入副本
            prompt_state = copy.deepcopy(start_state)
            self.model.forward(prompt_ids[0].tolist(), prompt_state)
            
            for _ in range(self.group_size):
                curr_state = copy.deepcopy(prompt_state)
                # 这里调用你原来的 generate_response 逻辑，但确保它使用传入的 curr_state
                res_text = generate_response(prompt, curr_state) 
                
                reward = 1.0 if verify_gsm8k_answer(res_text, gold_answer)['is_correct'] else 0.0
                samples.append({'text': res_text, 'reward': reward})

        # 2. 计算 GRPO 优势
        rewards = torch.tensor([s['reward'] for s in samples], device='cuda')
        advantages = (rewards - rewards.mean()) / (rewards.std() + 1e-8)

        # 3. 优化初始 State
        self.model.train()
        # 这里需要重新计算 log_probs，但在 State-tuning 中，
        # 我们优化的是影响全篇的“初始感官”，梯度会通过整个序列回传到 learnable_state
        
        total_loss = 0
        for i in range(self.group_size):
            # 获取带梯度的初始状态
            grad_state = self.model.get_trainable_state(bsz=1)
            
            # 计算 Log-probs (此时梯度会流向 grad_state)
            # 为了简化，我们只计算响应部分的平均 log_prob
            # 这里需要你实现一个带梯度的 forward_seq_logprobs
            log_p = self.get_sequence_logprob(prompt + samples[i]['text'], grad_state)
            
            loss = -(advantages[i] * log_p)
...
def load_gsm8k_train(file_path="gsm8k_train.jsonl"):
    """
    加载并预处理 GSM8K 训练数据
    """
    train_data = []
    if not os.path.exists(file_path):
        # 模拟数据提醒
        print(f"错误: 找不到训练文件 {file_path}")
        print("请确保数据格式为每行一个 JSON，包含 'question' 和 'answer' 键。")
        return []
    ...
    print(f"成功加载训练样本数: {len(train_data)}")
    return train_data
# --------------------------------------------------------------------------------
# 主循环
# --------------------------------------------------------------------------------
if __name__ == "__main__":
    trainer = GRPO_StateTuning_Trainer(model, tokenizer)
    
    # 模拟从数据集读取
    for epoch in range(10):
        print(f"Epoch {epoch}")
        # 假设 train_data 是你的 GSM8K 列表
        train_file = "C:\\RWKV-LM\\Albatross\\faster_251101\\gsm8k_test_formatted.jsonl" # 请确保此文件存在
        train_data = load_gsm8k_train(train_file)
        for item in tqdm(train_data):
            loss, avg_reward = trainer.train_step(item['question'], item['answer'])
            
            # 打印进度
            if avg_reward > 0:
                print(f" Found correct answer! Loss: {loss:.4f}, Reward: {avg_reward:.2f}")