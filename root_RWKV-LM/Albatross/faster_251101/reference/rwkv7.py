...
load(
    ...
    extra_cuda_cflags=[
        ...
        f"-D_N_={HEAD_SIZE}",
        "-allow-unsupported-compiler"  # 新增：跳过VS版本校验（解决核心报错）
...
class RWKV_x070(nn.Module):
    ...
    def init_state_tuning(self):
        """
        初始化 State-tuning：创建可学习的初始状态并冻结所有模型权重
        """
        # 1. 冻结所有模型权重
        ...
        args = self.args
        # 2. 创建可学习的 state 块
        # state[0]: [n_layer, 2, n_embd] -> 对应 TMix 的 x_prev 和 CMix 的 x_prev
        # state[1]: [n_layer, n_head, head_size, head_size] -> 对应 RWKV-7 的内部状态
        # 我们这里只训练 state[0] 和 state[1]
        
        self.learnable_state_0 = nn.Parameter(
            torch.zeros((args.n_layer, 2, args.n_embd), dtype=torch.float32, device="cuda")
        )
        self.learnable_state_1 = nn.Parameter(
            torch.zeros((args.n_layer, args.n_embd // args.head_size, args.head_size, args.head_size), 
                        dtype=torch.float32, device="cuda")
        )
        # state[2] 是时间偏移量，通常不需要训练，设为常量
        self.register_buffer('state_2', torch.zeros((), dtype=torch.int32, device="cuda"))

    def get_trainable_state(self, bsz):
        """
        根据 batch_size 复制初始状态
        """
        # 对 learnable_state 进行扩展以匹配 batch_size
        s0 = self.learnable_state_0.unsqueeze(2).expand(-1, -1, bsz, -1).contiguous()
        s1 = self.learnable_state_1.unsqueeze(1).expand(-1, bsz, -1, -1, -1).contiguous()
        s2 = self.state_2.expand(bsz).contiguous()
        return [s0, s1, s2]
...
def RWKV_x070_CMix_one(x, x_prev, x_k, K_, V_):
    ...
    # 🟢 智能判断：如果 flag_gems 加载了且包含这个稀疏算子，就用它；否则用普通乘法
    if hasattr(torch.ops.flag_gems, 'rwkv_mm_sparsity'):
