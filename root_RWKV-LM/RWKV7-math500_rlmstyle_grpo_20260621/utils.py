...
def build_prompt(problem: str, mode: str = "rwkv_boxed") -> str:
    ...
    if mode == "rlm_math":
        return (
            f"User: {p}\n"
            "Use the recursive decomposition workflow. First identify the smallest self-contained subproblems, "
            "solve them one by one, verify each intermediate value before using it, and then assemble the final answer. "
            "Keep a compact node/answer memo in the reasoning. Do not ramble or repeat. "
