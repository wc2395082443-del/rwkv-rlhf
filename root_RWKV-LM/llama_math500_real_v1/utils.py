...
def build_prompt(problem: str) -> str:
    """Build the math task content shared by plain and chat prompts."""
    p = (problem or "").strip()
    return (
        "Solve the following math problem carefully. "
        "Show the reasoning, and end with the final answer in \\boxed{...}.\n\n"
        f"Problem: {p}"
    )


def build_chat_messages(problem: str) -> List[Dict[str, str]]:
    """Build chat messages for instruct-style models."""
    return [
        {
            "role": "system",
            "content": "You are a careful math solver. Show the reasoning and end with the final answer in \\boxed{...}.",
        },
        {
            "role": "user",
            "content": build_prompt(problem),
        },
    ]
