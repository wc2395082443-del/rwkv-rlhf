...
class GRPOConfig:
    ...
    # REAL-style objective config
    policy_objective: str = 'grpo'
    real_tau: float = 0.5
    real_label_source: str = 'reward'
    real_reward_weight_cap: float = 2.0
...
class GRPOTrainer:
    ...
    def _sample_indices_for_train_step(self, exclude_indices: Optional[set[int]] = None) -> List[int]:
        need = max(1, int(self.cfg.num_questions))
        exclude = {int(x) for x in (exclude_indices or set())}
        pool = [i for i in range(len(self.train_data)) if i not in exclude]
        if len(pool) >= need:
            return self.rng.sample(pool, need)
        if pool:
            out = list(pool) if bool(getattr(self.cfg, "dynamic_unique_questions", True)) else []
            while len(out) < need:
                out.append(pool[self.rng.randrange(len(pool))])
            self.rng.shuffle(out)
            return out[:need]
        return [self.rng.randrange(len(self.train_data)) for _ in range(need)]
    ...
    def _assign_real_targets(self, group_infos: List[Dict[str, Any]]):
        label_source = str(getattr(self.cfg, 'real_label_source', 'reward'))
        weight_cap = max(1.0, float(getattr(self.cfg, 'real_reward_weight_cap', 2.0)))
        for group in group_infos:
            group_id = int(group.get('q_idx', 0))
            for traj in group.get('group_trajs', []):
                comps = traj.get('reward_components', {})
                reward_total = float(sum(float(v) for v in comps.values()))
                if label_source == 'correct':
                    label = 1.0 if bool(traj.get('is_correct', False)) else 0.0
                else:
                    label = 1.0 if reward_total > 0.0 else 0.0
                traj['group_id'] = group_id
                traj['real_reward_total'] = reward_total
                traj['real_label'] = label
                traj['real_weight'] = min(weight_cap, 1.0 + abs(reward_total))
                traj['advantage'] = reward_total

    def _optimize_trajs_real(self, trajs: List[Dict[str, Any]], lr_scale: float = 1.0) -> Dict[str, float]:
        if not trajs:
            return {
                'loss_total': 0.0,
                'kl_total': 0.0,
        ...
        batch_cnt = 0
        grad_norm = 0.0
        tau = max(1e-6, float(self.cfg.real_tau))

        grouped = {}
        for traj in trajs:
            grouped.setdefault(int(traj.get('group_id', 0)), []).append(traj)
        group_batches = list(grouped.values())
        ...
        try:
            for _ in range(self.cfg.ppo_epochs):
                ...
                total_groups = max(1, len(group_batches))
                for batch in group_batches:
                    batch_sorted = sorted(batch, key=lambda x: len(x['prompt_tokens']) + len(x['comp_tokens']), reverse=True)

                    pos_total = 0.0
                    neg_total = 0.0
                    valid_rollouts = 0

                    for start in range(0, len(batch_sorted), self.cfg.micro_batch):
                        sub_batch = batch_sorted[start:start + self.cfg.micro_batch]
                        seqs = [traj['prompt_tokens'] + traj['comp_tokens'] for traj in sub_batch]
                        ...
                        for bi, traj in enumerate(sub_batch):
                            ...
                            min_len = min(new_logp.size(0), old_logp.size(0))
                            if min_len == 0:
                                continue

                            seq_score = float((new_logp[:min_len] - old_logp[:min_len]).mean().item())
                            weight = float(traj.get('real_weight', 1.0))
                            if float(traj.get('real_label', 0.0)) > 0.5:
                                pos_total += math.exp(-seq_score / tau) * weight
                            else:
                                neg_total += math.exp(seq_score / tau) * weight
                            valid_rollouts += 1
                    ...
                    pos_denom = max(1e-6, 1.0 + pos_total)
                    neg_denom = max(1e-6, 1.0 + neg_total)
                    group_loss_value = math.log1p(pos_total) + math.log1p(neg_total)
                    group_kl_sum_value = 0.0
                    group_entropy_sum_value = 0.0
                    ...
                    for start in range(0, len(batch_sorted), self.cfg.micro_batch):
                        ...
                        sub_pos_sum = torch.zeros((), device=self.device, dtype=torch.float32)
                        sub_neg_sum = torch.zeros((), device=self.device, dtype=torch.float32)
                        sub_kl_sum = torch.zeros((), device=self.device, dtype=torch.float32)
                        sub_valid = 0
                        ...
                        for bi, traj in enumerate(sub_batch):
                            ...
                            seq_score = (new_logp - old_logp).mean()
                            seq_kl = compute_unbiased_kl(ref_logp, new_logp).mean()
                            seq_entropy = curr_entropy.mean()
                            weight = float(traj.get('real_weight', 1.0))
                            if float(traj.get('real_label', 0.0)) > 0.5:
                                sub_pos_sum = sub_pos_sum + (torch.exp(-seq_score / tau) * weight)
                            else:
                                sub_neg_sum = sub_neg_sum + (torch.exp(seq_score / tau) * weight)
                            sub_kl_sum = sub_kl_sum + seq_kl
                            group_kl_sum_value += float(seq_kl.item())
                            group_entropy_sum_value += float(seq_entropy.item())
                            sub_valid += 1

                        if sub_valid > 0:
                            total_loss = (sub_pos_sum / pos_denom + sub_neg_sum / neg_denom) / float(total_groups)
                            if self.cfg.kl_mode == 'k3_loss':
                                total_loss = total_loss + (self.cfg.kl_coef * (sub_kl_sum / float(valid_rollouts)) / float(total_groups))
                            total_loss.backward()

                        del logp, ref_logp_all, entropy_per_token

                    loss_total += group_loss_value
                    kl_total += (group_kl_sum_value / float(valid_rollouts))
                    entropy_total += (group_entropy_sum_value / float(valid_rollouts))
                    batch_cnt += 1
    ...
    def train_step(self, step: int) -> Dict[str, Any]:
        t0 = time.time()
        use_real = (str(getattr(self.cfg, 'policy_objective', 'grpo')).lower() == 'real')
        ...
        # Keep relative contribution scales aligned with original reward design at full magnitude.
        ...
        while True:
            sampling_rounds += 1
            sampled_indices = self._sample_indices_for_train_step(
            ...
            for q in sampled_questions:
                ids = self._encode_prompt_tokens(q.get("problem", ""))
                max_prompt_len = int(self.model.args.ctx_len) - int(self.cfg.max_new_tokens) - 4
                max_prompt_len = max(64, max_prompt_len)
                if len(ids) > max_prompt_len:
                    ids = ids[-max_prompt_len:]
        ...
        for group in group_infos:
            ...
            if (not use_real) and (group_all_wrong or group_all_correct):
                groups_skipped += 1
                if is_extra_group:
        ...
        if use_real:
            normal_opt = self._optimize_trajs_real(normal_trajs, lr_scale=1.0)
        else:
        ...
        if extra_step_ran:
            if use_real:
                extra_opt = self._optimize_trajs_real(extra_trajs, lr_scale=extra_lr_scale)
            else:
