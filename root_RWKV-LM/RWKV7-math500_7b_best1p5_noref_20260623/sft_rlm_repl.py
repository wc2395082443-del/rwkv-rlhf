...
SYSTEM = (
    ...
    'Solve by writing Python code inside ```repl blocks. The REPL has context, llm_query, '
    'llm_query_batched, SHOW_VARS(), and answer. To submit final answer, execute: '
    'answer["content"] = "\\\\boxed{...}"; answer["ready"] = True. Do not answer outside the REPL.\n\n'
)
...
def make_pair(problem, answer):
    ...
    comp = (
        '```repl\n'
        '# The answer is known from the supervised target; submit it through the RLM protocol.\n'
        f'answer["content"] = r"\\\\boxed{{{ans}}}"\n'
        'answer["ready"] = True\n'
        '```'
    )
    return prompt, comp
...
def pad(seqs, pad=0):
    ...
    for i,x in enumerate(seqs): out[i,:len(x)]=torch.tensor(x,dtype=torch.long)
    return out

def main():
    ap=argparse.ArgumentParser()
    ap.add_argument('--model', required=True); ap.add_argument('--tokenizer', required=True); ap.add_argument('--train_jsonl', required=True); ap.add_argument('--out_dir', required=True)
    ap.add_argument('--ctx_len', type=int, default=8192); ap.add_argument('--model_dtype', default='bf16'); ap.add_argument('--steps', type=int, default=100); ap.add_argument('--batch_size', type=int, default=1); ap.add_argument('--lr', type=float, default=2e-6); ap.add_argument('--max_samples', type=int, default=2000); ap.add_argument('--save_interval', type=int, default=50); ap.add_argument('--seed', type=int, default=42)
    ...
    tok=TRIE_TOKENIZER(args.tokenizer); encode=lambda s: tok.encode(s)
    base,pth=normalize_model_arg(args.model)
    model,_=load_train_model_rwkv7_cuda(pth,'cuda',args.ctx_len,'fullstate',args.model_dtype)
    unfreeze_all_parameters(model); model.train()
    ...
    pairs=[]
    for ex in data:
        problem=ex.get('problem',''); ans=ex.get('answer', ex.get('solution',''))
        pr,co=make_pair(problem, ans)
        pids=encode(pr); cids=encode(co)
        max_len=args.ctx_len-8
        if len(pids)+len(cids)>max_len:
            pids=pids[-max(64,max_len-len(cids)):]
        pairs.append((pids,cids,problem))
    logp=os.path.join(args.out_dir,'sft_metrics.jsonl')
    t0=time.time()
    for step in range(1,args.steps+1):
        batch=rng.sample(pairs, min(args.batch_size,len(pairs)))
        seqs=[]; masks=[]
        for pids,cids,_ in batch:
            ids=pids+cids
            mask=[0]*len(pids)+[1]*len(cids)
            seqs.append(ids); masks.append(mask)
        x=pad(seqs).cuda(); mask=pad(masks).cuda().float()
        ...
        with open(logp,'a',encoding='utf-8') as f: f.write(json.dumps(rec)+'\n')
        print(f"[SFT {step}/{args.steps}] loss={rec['loss']:.4f} tok={rec['tokens']} elapsed={rec['elapsed']:.1f}s", flush=True)
        if args.save_interval>0 and step%args.save_interval==0:
            ck=os.path.join(args.out_dir,f'ckpt_sft_step{step}.pth')
    ...
    ck=os.path.join(args.out_dir,f'ckpt_sft_step{args.steps}.pth')
    torch.save({'step':args.steps,'model':{n:p.detach().cpu() for n,p in model.named_parameters() if p.requires_grad}}, ck)
    print('FINAL_CKPT',ck)
