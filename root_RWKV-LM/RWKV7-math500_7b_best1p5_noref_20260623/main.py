...
def parse_args():
    ...
    ap.add_argument("--prompt_mode", type=str, default="trl_doc", choices=["rwkv_boxed", "trl_doc", "question_only", "short_math", "recursive_math", "rlm_repl"], help="?????")
    ...
    ap.add_argument("--optimizer", type=str, default="adam", choices=["adam", "sgd", "paged_adamw_8bit", "bnb_paged_adamw8bit", "bnb8bit", "cpu_adam_fp32", "cpuadam", "cpu_adam"], help="optimizer")
...
def main():
    ...
    need_ref_model = (abs(float(args.kl_coef)) > 0.0) or (str(args.kl_mode) == "k3_loss")
    if need_ref_model:
        ...
        if args.state_init:
            ...
            print(f"load ref time_state: {ok} (from {args.state_init})")
        ref_model.eval()
        for p in ref_model.parameters():
            p.requires_grad = False
    else:
        print("skip reference model: kl_coef=0 and kl_mode is not k3_loss")
        ref_model = None
