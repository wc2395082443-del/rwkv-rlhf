...
class AlbatrossBatchInference:
    ...
    def _forward_stateful_active(self, step_tokens: List[List[int]], state, active_idx: List[int]) -> torch.Tensor:
        ...
        chunk_size = max(1, int(getattr(self.cfg, "rollout_forward_batch", 8)))
        outputs = []
        for start in range(0, len(active_idx), chunk_size):
            part_idx = active_idx[start:start + chunk_size]
            part_tokens = step_tokens[start:start + chunk_size]
            idx_tensor = torch.tensor(part_idx, device=self.device, dtype=torch.long)
            state0 = state[0].index_select(2, idx_tensor).contiguous()
            state1 = state[1].index_select(1, idx_tensor).contiguous()
            next_logits = self.infer_model.forward_batch(part_tokens, [state0, state1])
            ...
            del state0, state1, idx_tensor, next_logits
        return torch.cat(outputs, dim=0)
            ...
            for row, tok, lp in zip(idx_cpu, tok_cpu, lp_cpu):
                ...
                if (t < 2) or (len(comp_tokens[row]) % text_check_interval == 0):
                    if stop_on_think_close and "</think>" in text:
                        active[row] = False
                        continue
                    if stop_strings and any(ss in text for ss in stop_strings):
                        active[row] = False
                        continue
