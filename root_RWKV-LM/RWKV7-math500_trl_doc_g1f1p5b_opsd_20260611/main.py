...
def parse_args():
    ...
    ap.add_argument("--teacher_model", type=str, default="", help="OPD teacher????(??)")
    ...
    ap.add_argument("--opd_top_k", type=int, default=0, help="teacher top-k tokens for OPD KL; 0 = full vocab")
    ap.add_argument("--opd_correct_only", type=int, default=0, help="apply OPD only to correct positive-advantage trajectories")
    ap.add_argument("--opd_mixed_only", type=int, default=0, help="apply OPD only to mixed all0/all1-filtered groups")
    ap.add_argument("--opsd_coef", type=float, default=0.0, help="OPSD self-distillation loss weight")
    ap.add_argument("--opsd_temp", type=float, default=1.0, help="OPSD softmax temperature")
    ap.add_argument("--opsd_top_k", type=int, default=64, help="teacher top-k tokens for OPSD KL; 0 = full vocab")
    ap.add_argument("--opsd_correct_only", type=int, default=1, help="apply OPSD only to correct positive-advantage trajectories")
    ap.add_argument("--opsd_mixed_only", type=int, default=1, help="apply OPSD only to mixed groups")
    ap.add_argument("--opsd_agreement_gate", type=int, default=1, help="apply OPSD only when hint branch likes sampled token more")
    ap.add_argument("--opsd_margin", type=float, default=0.0, help="teacher_action_logp >= student_action_logp + margin")
...
def main():
    ...
    if float(args.opd_coef) > 0.0 and str(args.teacher_model).strip():
        teacher_base_name, teacher_pth_path = normalize_model_arg(str(args.teacher_model).strip())
    ...
    cfg = GRPOConfig(
        ...
        opd_top_k=int(args.opd_top_k),
        opd_correct_only=bool(int(args.opd_correct_only)),
        opd_mixed_only=bool(int(args.opd_mixed_only)),
        opsd_coef=float(args.opsd_coef),
        opsd_temp=float(args.opsd_temp),
        opsd_top_k=int(args.opsd_top_k),
        opsd_correct_only=bool(int(args.opsd_correct_only)),
        opsd_mixed_only=bool(int(args.opsd_mixed_only)),
        opsd_agreement_gate=bool(int(args.opsd_agreement_gate)),
        opsd_margin=float(args.opsd_margin),
