...
class GRPOConfig:
    ...
    opd_top_k: int = 0
    opd_correct_only: bool = False
    opd_mixed_only: bool = False
    opsd_coef: float = 0.0
    opsd_temp: float = 1.0
    opsd_top_k: int = 64
    opsd_correct_only: bool = True
    opsd_mixed_only: bool = True
    opsd_agreement_gate: bool = True
    opsd_margin: float = 0.0
...
class GRPOTrainer:
    ...
    def _build_opsd_prompt_tokens(self, problem: str, answer: str) -> List[int]:
        p = (problem or "").strip()
        a = (answer or "").strip()
        prompt = (
            f"User: {p}\n\n"
            f"Training hint: the verified final answer is {a}. "
            f"Use the hint to check the reasoning and end with \\boxed{{{a}}}.\n\n"
    ...
    def _optimize_trajs(self, trajs: List[Dict[str, Any]], lr_scale: float = 1.0, adv_clip: Optional[float] = None) -> Dict[str, float]:
        ...
        opd_top_k = max(0, int(getattr(self.cfg, "opd_top_k", 0)))
        opd_correct_only = bool(getattr(self.cfg, "opd_correct_only", False))
        opd_mixed_only = bool(getattr(self.cfg, "opd_mixed_only", False))
        opsd_coef = float(getattr(self.cfg, "opsd_coef", 0.0))
        opsd_temp = max(1e-6, float(getattr(self.cfg, "opsd_temp", 1.0)))
        opsd_top_k = max(0, int(getattr(self.cfg, "opsd_top_k", 64)))
        opsd_correct_only = bool(getattr(self.cfg, "opsd_correct_only", True))
        opsd_mixed_only = bool(getattr(self.cfg, "opsd_mixed_only", True))
        opsd_agreement_gate = bool(getattr(self.cfg, "opsd_agreement_gate", True))
        opsd_margin = float(getattr(self.cfg, "opsd_margin", 0.0))
        use_opsd = opsd_coef > 0.0
        ...
        try:
            for _ in range(self.cfg.ppo_epochs):
                ...
                for start in range(0, len(trajs_sorted), self.cfg.micro_batch):
                    ...
                    batch_opsd = torch.zeros((), device=self.device, dtype=torch.float32)
                    batch_opsd_tokens = 0
                    batch_opsd_eligible_tokens = 0
                    total_tokens = 0
                    ...
                    for bi, traj in enumerate(batch):
                        ...
                        opd_apply = use_opd
                        if opd_apply and opd_correct_only:
                            opd_apply = bool(traj.get("is_correct", False)) and adv_val > 0.0
                        if opd_apply and opd_mixed_only:
                            opd_apply = bool(traj.get("opd_group_mixed", True))
                        opsd_apply = use_opsd
                        if opsd_apply and opsd_correct_only:
                            opsd_apply = bool(traj.get("is_correct", False)) and adv_val > 0.0
                        if opsd_apply and opsd_mixed_only:
                            opsd_apply = bool(traj.get("opd_group_mixed", True))
                        opsd_prompt_tokens = None
                        if opsd_apply:
                            opsd_prompt_tokens = traj.get("opsd_prompt_tokens")
                        ...
                        for chunk_start in range(0, min_len, chunk_tokens):
                            ...
                            if teacher_hidden is not None and opd_apply:
                                with torch.no_grad():
                                ...
                                if opd_top_k > 0 and opd_top_k < teacher_logits_f.size(-1):
                                    top_vals, top_idx = torch.topk(teacher_logits_f, k=opd_top_k, dim=-1)
                                    student_logprob_full = F.log_softmax(student_logits_f / opd_temp, dim=-1)
                                    student_logprob_t = student_logprob_full.gather(-1, top_idx)
                                    teacher_prob_t = F.softmax(top_vals / opd_temp, dim=-1)
                                    opd_loss = F.kl_div(student_logprob_t, teacher_prob_t, reduction='sum') * (opd_temp * opd_temp)
                                    del top_vals, top_idx, student_logprob_full
                                else:
                            ...
                            opsd_loss = torch.zeros((), device=self.device, dtype=torch.float32)
                            if opsd_apply and opsd_prompt_tokens is not None:
                                opsd_seq = list(opsd_prompt_tokens) + list(traj['comp_tokens'])
                                opsd_inp = torch.tensor(opsd_seq[:-1], dtype=torch.long, device=self.device).unsqueeze(0)
                                opsd_start_idx = len(opsd_prompt_tokens) - 1 + chunk_start
                                opsd_end_idx = len(opsd_prompt_tokens) - 1 + chunk_end
                                with torch.no_grad():
                                    opsd_hidden = self.model.forward_hidden(opsd_inp)
                                    if torch.is_tensor(opsd_hidden) and opsd_hidden.dim() == 2:
                                        opsd_hidden = opsd_hidden.unsqueeze(0)
                                    opsd_logits = self.model.project_logits(opsd_hidden[:, opsd_start_idx:opsd_end_idx, :])
                                opsd_teacher_logits_f = opsd_logits.squeeze(0).float()
                                opsd_student_logits_f = student_logits.squeeze(0).float()
                                tgt_flat = tgt_chunk.squeeze(0)
                                student_logprob_full = F.log_softmax(opsd_student_logits_f / opsd_temp, dim=-1)
                                teacher_logprob_full = F.log_softmax(opsd_teacher_logits_f / opsd_temp, dim=-1)
                                gate = torch.ones((tgt_flat.numel(),), device=self.device, dtype=torch.bool)
                                if opsd_agreement_gate:
                                    student_action_logp = student_logprob_full.gather(-1, tgt_flat.view(-1, 1)).squeeze(-1)
                                    teacher_action_logp = teacher_logprob_full.gather(-1, tgt_flat.view(-1, 1)).squeeze(-1)
                                    gate = gate & (teacher_action_logp >= (student_action_logp + opsd_margin))
                                if opsd_top_k > 0 and opsd_top_k < opsd_teacher_logits_f.size(-1):
                                    top_vals, top_idx = torch.topk(opsd_teacher_logits_f, k=opsd_top_k, dim=-1)
                                    if opsd_agreement_gate:
                                        in_top = (top_idx == tgt_flat.view(-1, 1)).any(dim=-1)
                                        gate = gate & in_top
                                    if bool(gate.any().item()):
                                        selected_student_logprob = student_logprob_full[gate].gather(-1, top_idx[gate])
                                        selected_teacher_prob = F.softmax(top_vals[gate] / opsd_temp, dim=-1)
                                        opsd_loss = F.kl_div(selected_student_logprob, selected_teacher_prob, reduction='sum') * (opsd_temp * opsd_temp)
                                        batch_opsd_tokens += int(gate.sum().item())
                                    del top_vals, top_idx
                                else:
                                    if bool(gate.any().item()):
                                        selected_student_logprob = student_logprob_full[gate]
                                        selected_teacher_prob = F.softmax(opsd_teacher_logits_f[gate] / opsd_temp, dim=-1)
                                        opsd_loss = F.kl_div(selected_student_logprob, selected_teacher_prob, reduction='sum') * (opsd_temp * opsd_temp)
                                        batch_opsd_tokens += int(gate.sum().item())
                                batch_opsd_eligible_tokens += int(tgt_flat.numel())
                                del opsd_inp, opsd_hidden, opsd_logits, opsd_teacher_logits_f, opsd_student_logits_f, tgt_flat, student_logprob_full, teacher_logprob_full, gate
                            ...
                            batch_opsd = batch_opsd + opsd_loss
                    ...
                    if total_tokens > 0:
                        ...
                        normalized_opsd = batch_opsd / max(1, batch_opsd_tokens)
                        ...
                        if use_opsd and batch_opsd_tokens > 0:
                            total_loss = total_loss + opsd_coef * normalized_opsd
                        ...
                        opsd_total += normalized_opsd.item() if batch_opsd_tokens > 0 else 0.0
                        opsd_tokens += int(batch_opsd_tokens)
                        opsd_eligible_tokens += int(batch_opsd_eligible_tokens)
        ...
        return {
            ...
            'grad_norm': grad_norm,
            'opd_total': opd_total,
            'opsd_total': opsd_total,
            'opsd_tokens': opsd_tokens,
            'opsd_eligible_tokens': opsd_eligible_tokens,
        }
    ...
    def train_step(self, step: int) -> Dict[str, Any]:
        ...
        def _consume_sample(group, comp_tokens, old_logps, comp_text, truncated, is_extra: bool = False):
            ...
            if float(getattr(self.cfg, "opsd_coef", 0.0)) > 0.0:
                traj["opsd_prompt_tokens"] = self._build_opsd_prompt_tokens(group.get("problem", ""), group.get("answer", ""))
        ...
        for group in group_infos:
            ...
            for traj in group.get("group_trajs", []):
                traj["opd_group_mixed"] = not (group_all_wrong or group_all_correct)
        ...
        opsd_total = float(normal_opt.get('opsd_total', 0.0) + extra_opt.get('opsd_total', 0.0))
        opsd_tokens = int(normal_opt.get('opsd_tokens', 0) + extra_opt.get('opsd_tokens', 0))
        opsd_eligible_tokens = int(normal_opt.get('opsd_eligible_tokens', 0) + extra_opt.get('opsd_eligible_tokens', 0))
        ...
        metrics = {
            ...
            "opsd_loss": opsd_total,
            "avg_opsd_loss": (opsd_total / max(1, batch_cnt)),
            "opsd_tokens": opsd_tokens,
            "opsd_eligible_tokens": opsd_eligible_tokens,
            "opsd_gate_rate": (opsd_tokens / max(1, opsd_eligible_tokens)),
            "avg_zstd_penalty": avg_zstd_penalty,
            "avg_zstd_ratio": avg_zstd_ratio,
        ...
        for step in range(1, total_steps + 1):
            ...
            if step % self.cfg.log_interval == 0:
                self._log(
                    ...
                    f"opsd={metrics.get('avg_opsd_loss', 0.0):.4f} "
                    f"opsd_gate={metrics.get('opsd_gate_rate', 0.0):.3f} "
