...
def _clean_text(s: str, limit: int = 2200) -> str:
    s = str(s or '').replace('\r\n', '\n').replace('\r', '\n').strip()
    # Keep the trace bounded; the REPL should teach the protocol, not overflow ctx.
    if len(s) > limit:
        s = s[:limit].rstrip() + ' ...'
    return s


def make_pair(problem, answer, solution=''):
    ...
    # This is a protocol-and-reasoning trace: read context, keep a derivation in the
    # REPL state, print it for the environment, then submit via answer[ready].
    code = (
        'problem = context\n'
        f'reasoning = {sol!r}\n'
        'print(reasoning)\n'
        f'answer["content"] = {f"\\\\boxed{{{ans}}}"!r}\n'
        'answer["ready"] = True\n'
    )
    comp = '```repl\n' + code + '```'
    return prompt, comp
...
def main():
    ...
    for ex in data:
        ...
        ans = ex.get('answer', ex.get('solution', ''))
        sol = ex.get('solution', ans)
        pr, co = make_pair(problem, ans, sol)
    ...
    print(f'built_pairs={len(pairs)} dropped={dropped}', flush=True)
    ...
    for step in range(1, args.steps + 1):
        ...
        print(f"[SFT-SOL {step}/{args.steps}] loss={rec['loss']:.4f} tok={rec['tokens']} elapsed={rec['elapsed']:.1f}s", flush=True)
