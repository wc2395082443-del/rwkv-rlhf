...
def build_prompt(problem: str, mode: str = "rwkv_boxed") -> str:
    ...
    if mode == "rlm_repl":
        return (
            "You are a Recursive Language Model with a local Python REPL. "
            "Use ```repl blocks, inspect context, optionally call llm_query, "
            "and submit answer[\"content\"] then answer[\"ready\"]=True.\n"
        )
    if mode == "recursive_math":
        return (
            f"User: {p}\n"
            "We will solve this with an external recursive controller. Start by making a compact decomposition, "
            "then solve subproblems, then assemble the final answer in \boxed{...}.\n"
            f"Assistant: <think>\n"
        )
    if mode == "short_math":
        return (
