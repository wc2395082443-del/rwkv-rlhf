...
def parse_args():
    ...
    ap.add_argument("--dynamic_min_valid_groups", type=int, default=16, help="min valid groups after dynamic resampling")
    ap.add_argument("--dynamic_max_resample_rounds", type=int, default=2, help="max extra sampling rounds")
    ap.add_argument("--dynamic_max_keep_groups", type=int, default=24, help="max valid groups kept for optimization (0=all)")
    ...
    ap.add_argument("--answer_judge", type=str, default="legacy", choices=["legacy", "math_verify", "auto"], help="?????")
    ...
    # 其他
    ap.add_argument("--alternate_datasets", action="store_true", help="按_dataset字段在step间交替采样")
    ap.add_argument("--alternate_dataset_a", type=str, default="math500", help="交替采样的数据集A")
    ap.add_argument("--alternate_dataset_b", type=str, default="gsm8k", help="交替采样的数据集B")
...
def main():
    ...
    cfg = GRPOConfig(
        ...
        dynamic_min_valid_groups=int(args.dynamic_min_valid_groups),
        dynamic_max_resample_rounds=int(args.dynamic_max_resample_rounds),
        dynamic_max_keep_groups=int(args.dynamic_max_keep_groups),
        alternate_datasets=bool(args.alternate_datasets),
        alternate_dataset_a=str(args.alternate_dataset_a),
        alternate_dataset_b=str(args.alternate_dataset_b),
    )
