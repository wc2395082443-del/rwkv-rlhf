...
class GRPOConfig:
    ...
    alternate_datasets: bool = False
    alternate_dataset_a: str = "math500"
    alternate_dataset_b: str = "gsm8k"
...
class GRPOTrainer:
    def __init__(
        ...
        for idx, item in enumerate(self.train_data):
            ds = str(item.get("_dataset", "")).strip()
            if ds:
                self.dataset_to_indices.setdefault(ds, []).append(idx)
    ...
    def train_step(self, step: int) -> Dict[str, Any]:
        ...
        def _sample_question_indices():
            active_indices = None
            active_name = None
            if self.cfg.alternate_datasets and self.dataset_to_indices:
                order = [self.cfg.alternate_dataset_a, self.cfg.alternate_dataset_b]
                active_name = order[(step - 1) % len(order)]
                active_indices = self.dataset_to_indices.get(active_name)
                if not active_indices:
                    active_name = None
                    active_indices = None
            pool = active_indices if active_indices else list(range(len(self.train_data)))
            if len(pool) >= self.cfg.num_questions:
                picked = self.rng.sample(pool, self.cfg.num_questions)
            else:
                picked = [pool[self.rng.randrange(len(pool))] for _ in range(self.cfg.num_questions)]
            return picked, active_name or "mixed"

        group_infos = []
        step_dataset_name = "mixed"
        ...
        sample_indices, step_dataset_name = _sample_question_indices()
        _append_sampling_round(sample_indices)
        ...
        metrics = {
            ...
            "step_dataset": step_dataset_name,
        }
        ...
        for step in range(1, total_steps + 1):
            ...
            if step % self.cfg.log_interval == 0:
                self._log(
                    ...
                    f"ds={metrics.get('step_dataset', 'mixed')} "
