            ...
            for _ in range(self.cfg.ppo_epochs):
                ...
                for start in range(0, len(trajs_sorted), self.cfg.micro_batch):
                    ...
                    with torch.no_grad():
                        ref_chunks = []
                        ref_chunk_bs = 1
                        for ref_start in range(0, inp.size(0), ref_chunk_bs):
                            inp_chunk = inp[ref_start:ref_start + ref_chunk_bs]
                            tgt_chunk = tgt[ref_start:ref_start + ref_chunk_bs]
                            ref_logits = self.ref_model(inp_chunk)
                            if torch.is_tensor(ref_logits) and ref_logits.dim() == 2:
                                ref_logits = ref_logits.unsqueeze(0)
                            ref_logp_chunk = self._logp_with_sampling(ref_logits, tgt_chunk)
                            ref_chunks.append(ref_logp_chunk)
                            del inp_chunk, tgt_chunk, ref_logits, ref_logp_chunk
                            torch.cuda.empty_cache()
                        ref_logp_all = torch.cat(ref_chunks, dim=0)
                        del ref_chunks
                        torch.cuda.empty_cache()
