...
CHOICE_ANSWERS = {chr(ord("A") + i) for i in range(10)}
...
def _extract_choice_answer(text: str) -> Optional[str]:
    ...
    patterns = [
        r"answer\s*is\s*\(?\s*([A-Ja-j])\s*\)?",
        r"answer\s*:\s*\(?\s*([A-Ja-j])\s*\)?",
        r"final\s*answer\s*is\s*\(?\s*([A-Ja-j])\s*\)?",
    ]
...
def extract_answer(text: str, ground_truth: str = None) -> Optional[str]:
    ...
    gt_choice = str(ground_truth).strip().upper() if ground_truth is not None else ""
    if gt_choice in CHOICE_ANSWERS:
        return _extract_choice_answer(text)
...
def check_answer_correct(extracted: str, ground_truth: str) -> bool:
    ...
    if str(gt_extracted).strip().upper() in CHOICE_ANSWERS:
        return str(extracted).strip().upper() == str(gt_extracted).strip().upper()
...
def check_format_correct(text: str, ground_truth: str) -> bool:
    ...
    if str(gt_extracted).strip().upper() in CHOICE_ANSWERS:
        boxed = extract_last_boxed(text)
        return boxed is not None and str(boxed).strip().upper() == str(gt_extracted).strip().upper()
