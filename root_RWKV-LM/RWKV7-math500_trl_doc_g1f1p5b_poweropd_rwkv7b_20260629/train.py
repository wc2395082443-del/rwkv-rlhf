...
class GRPOConfig:
    ...
    opd_mode: str = "tokenkl"
    poweropd_alpha: float = 0.0
...
def compute_poweropd_reward(teacher_logp: torch.Tensor, student_logp: torch.Tensor, alpha: float) -> torch.Tensor:
    alpha = float(alpha)
    teacher_logp = teacher_logp.detach().float().clamp(min=-60.0, max=0.0)
    student_logp = student_logp.detach().float().clamp(min=-60.0, max=0.0)
    if abs(alpha) < 1e-8:
        return teacher_logp - student_logp
    teacher_prob = torch.exp(teacher_logp)
    student_prob = torch.exp(student_logp)
    return (teacher_prob.pow(alpha) - student_prob.pow(alpha)) / alpha
...
class GRPOTrainer:
    ...
    @torch.no_grad()
    def _teacher_forward_hidden(self, input_ids: torch.Tensor) -> Optional[torch.Tensor]:
        if not self._teacher_ready():
            return None
        self.teacher_model.eval()
        if self.teacher_kind == "rwkv":
            hidden = self.teacher_model.forward_hidden(input_ids)
        else:
    ...
    def _assign_opd_advantages(self, group_infos: List[Dict[str, Any]]) -> Dict[str, float]:
        # Token-level distillation no longer uses sequence-level teacher reranking.
        pair_count = 0
        for group in group_infos:
            ...
            for traj in group.get('group_trajs', []):
                ...
                pair_count += 1
        return {'opd_pairs': pair_count, 'opd_score_mean': 0.0, 'opd_score_std': 0.0}
                ...
                for start in range(0, len(trajs_sorted), self.cfg.micro_batch):
                    ...
                    with torch.no_grad():
                        ...
                        if self._teacher_ready():
                            teacher_hidden = self._teacher_forward_hidden(inp)
                        torch.cuda.empty_cache()
                    ...
                    for bi, traj in enumerate(batch):
                        ...
                        if self._teacher_ready() and teacher_hidden is not None:
                            teacher_hidden_slice = teacher_hidden[bi:bi+1, start_idx:end_idx, :][:, :min_len, :]
                            student_slice = self._last_policy_logits[bi, start_idx:end_idx, :][:min_len]
                            if teacher_hidden_slice.size(1) > 0 and student_slice.size(0) > 0:
                                opd_mode = str(getattr(self.cfg, 'opd_mode', 'tokenkl')).lower()
                                chunk_tokens = max(1, int(getattr(self.cfg, 'logit_chunk_tokens', 128)))
                                sampled_ids_full = tgt[bi, start_idx:end_idx][:min_len]
                                ...
                                for chunk_start in range(0, min_len, chunk_tokens):
                                    ...
                                    teacher_hidden_chunk = teacher_hidden_slice[:, chunk_start:chunk_end, :]
                                    with torch.no_grad():
                                        teacher_logits_chunk = self.teacher_model.project_logits(teacher_hidden_chunk)
                                    teacher_slice = teacher_logits_chunk.squeeze(0).float()
                                    student_chunk = student_slice[chunk_start:chunk_end].float()
                                    if opd_mode == 'power':
                                        sampled_ids = sampled_ids_full[chunk_start:chunk_end]
                                        teacher_logp_tokens = F.log_softmax(teacher_slice, dim=-1).gather(-1, sampled_ids.unsqueeze(-1)).squeeze(-1)
                                        power_reward = compute_poweropd_reward(
                                            teacher_logp_tokens,
                                            old_logp[chunk_start:chunk_end],
                                            float(getattr(self.cfg, 'poweropd_alpha', 0.0)),
                                        )
                                        opd_loss = opd_loss - torch.min(
                                            ratio[chunk_start:chunk_end] * power_reward,
                                            ratio_clipped[chunk_start:chunk_end] * power_reward,
                                        ).sum() / max(1, min_len)
                                    else:
                                        distill_temp = max(1e-5, float(getattr(self.cfg, 'opd_temperature', 1.0)))
                                        s_log_probs = F.log_softmax(student_chunk / distill_temp, dim=-1)
                                        t_probs = F.softmax(teacher_slice / distill_temp, dim=-1)
                                        opd_loss = opd_loss + F.kl_div(s_log_probs, t_probs, reduction='sum') * (distill_temp * distill_temp) / max(1, min_len)
                                    del teacher_logits_chunk, teacher_slice, student_chunk
                                batch_opd_loss += opd_loss
                                batch_opd_items += 1
                    ...
                    if total_tokens > 0:
                        ...
                        if teacher_hidden is not None:
                            del teacher_hidden
                        if hasattr(self, '_last_policy_logits'):
                            del self._last_policy_logits
                        ...
                        opd_loss_total += float(normalized_opd.item() if torch.is_tensor(normalized_opd) else normalized_opd)
                        batch_cnt += 1
    ...
    def train_step(self, step: int) -> Dict[str, Any]:
        ...
        _sync_cuda()
        _t = time.time()
        opd_only_opt = {'opd_loss_total': 0.0, 'batch_cnt': 0, 'grad_norm': 0.0, 'phase_times': {'zero_grad_time': 0.0, 'pad_time': 0.0, 'policy_forward_time': 0.0, 'loss_time': 0.0, 'backward_time': 0.0, 'grad_clip_time': 0.0, 'opt_step_time': 0.0}}
        _sync_cuda()
