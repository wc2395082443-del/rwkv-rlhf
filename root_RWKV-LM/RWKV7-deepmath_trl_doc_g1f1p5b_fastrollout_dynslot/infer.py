...
class AlbatrossBatchInference:
    ...
    @torch.no_grad()
    def _forward_stateful_dense(self, step_token_ids: List[int], state) -> torch.Tensor:
        token_tensor = torch.tensor(step_token_ids, device=self.device, dtype=torch.long).view(-1, 1)
        next_logits = self.infer_model.forward_batch(token_tensor, state)
    ...
    def generate_group_parallel(
        ...
        for t in range(max_new_tokens):
            ...
            if full_rollout:
                ...
                dense_step_ids = [0] * B
                for row, tok in zip(next_rows, [x[0] for x in step_tokens_batch]):
                    dense_step_ids[row] = int(tok)
                last_logits = self._forward_stateful_dense(dense_step_ids, state)
