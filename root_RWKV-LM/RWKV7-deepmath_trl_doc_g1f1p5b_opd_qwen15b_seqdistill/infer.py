...
def sample_next_token(
    ...
    original_logp = F.log_softmax(logits_f, dim=-1)

    sampling_logits = logits_f
    use_penalty = (token_counts is not None) and (presence_penalty > 0 or frequency_penalty > 0)
    if use_penalty:
    ...
    fast_path = (not use_penalty) and temperature == 1.0 and top_k <= 0 and (top_p <= 0.0 or top_p >= 1.0)
    if fast_path:
        probs = original_logp.exp()
    else:
        sampling_logits = apply_temperature(sampling_logits, temperature)
        sampling_logits = apply_top_k(sampling_logits, top_k)
...
class AlbatrossBatchInference:
    ...
    @torch.no_grad()
    def _forward_stateful_active(self, step_tokens, state, active_idx) -> torch.Tensor:
        if torch.is_tensor(active_idx):
            ...
            idx_tensor = active_idx.to(device=self.device, dtype=torch.long)
        else:
            if not active_idx:
                return torch.empty((0, self.train_model.args.vocab_size), dtype=torch.float32, device=self.device)
        ...
        if len(state) > 2:
            sliced = [
                state[0].index_select(2, idx_tensor).contiguous(),
                state[1].index_select(1, idx_tensor).contiguous(),
                state[2].index_select(0, idx_tensor).contiguous(),
            ]
        else:
            sliced = [
                state[0].index_select(2, idx_tensor).contiguous(),
        ...
        next_logits = self.infer_model.forward_batch(step_tokens, sliced)
        if torch.is_tensor(next_logits) and next_logits.dim() == 3:
            next_logits = next_logits[:, -1, :]
        state[0][:, :, idx_tensor] = sliced[0]
        state[1][:, idx_tensor] = sliced[1]
        if len(state) > 2:
            state[2][idx_tensor] = sliced[2]
        return next_logits
    ...
    def generate_group_parallel(
        ...
        use_penalty = (presence_penalty > 0.0) or (frequency_penalty > 0.0)
        token_counts = torch.zeros((B, last_logits.size(-1)), device=last_logits.device) if use_penalty else None
        need_incremental_text = stop_on_think_close or stop_on_boxed or stop_on_repeat_ngram or (stop_on_user and not self._stop_token_id_seqs)
        ...
        for t in range(max_new_tokens):
            ...
            cur_counts = token_counts.index_select(0, active_idx) if use_penalty else None
            ...
            for pos, (row, tok, lp) in enumerate(zip(idx_cpu, tok_cpu, lp_cpu)):
            ...
            keep_pos_tensor = torch.tensor(keep_pos, device=last_logits.device, dtype=torch.long)
            next_idx = active_idx.index_select(0, keep_pos_tensor)

            if full_rollout:
                next_rows = next_idx.detach().cpu().tolist()
                next_toks = token_ids.index_select(0, keep_pos_tensor).detach().cpu().tolist()
                for row, tok in zip(next_rows, next_toks):
                    full_sequences[row].append(int(tok))
                new_logits = self._forward_last_logits_full([full_sequences[row] for row in next_rows])
            else:
                next_rows = next_idx.detach().cpu().tolist()
                step_tokens_batch = [[int(tok_cpu[pos])] for pos in keep_pos]
                new_logits = self._forward_stateful_active(step_tokens_batch, state, next_idx)
