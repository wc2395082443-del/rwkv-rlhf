...
while pgrep -f "train_rl_dynamic.py --load_model" >/dev/null; do
  sleep 60
  date '+waiting %F %T'
done
out="/root/autodl-tmp/fp32_baseline_trial_20260417_173737/run"
mkdir -p "$out"
/root/miniconda3/bin/python train_rl_baseline.py \
  ...
  --devices 1 --accelerator gpu --strategy deepspeed_stage_3_offload --precision 32 \
  ...
  --lr 5e-7 --neg_adv_weight 0.6 --kl_coef 0.0 \
