...
# RWKV takeoff does not use LoRA, but verl imports this module unconditionally.
# The local minimal vLLM-RWKV build skips FlashAttention/LoRA-heavy extension
# paths, so keep the non-LoRA training path importable.
try:
    try:
...
except Exception as exc:  # pragma: no cover - environment compatibility shim
    _LORA_AVAILABLE = False
    _LORA_IMPORT_ERROR = exc

    class LoRAModel:  # type: ignore[no-redef]
        pass

    class LoRARequest:  # type: ignore[no-redef]
        pass

    def get_adapter_absolute_path(lora_path):  # type: ignore[no-redef]
        return lora_path

    LRUCacheWorkerLoRAManager = None  # type: ignore[assignment]
...
class VLLMHijack:
    @staticmethod
    def hijack():
        if not _LORA_AVAILABLE or LRUCacheWorkerLoRAManager is None:
            return
