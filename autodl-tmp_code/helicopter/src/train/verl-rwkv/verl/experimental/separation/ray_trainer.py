...
class SeparateRayPPOTrainer(RayPPOTrainer):
    ...
    def _fit_update_weights(self):
        # The one-step separation worker is actor-only in the single-GPU smoke path;
        # official weight sync requires a separate rollout worker/resource split.
        if os.getenv("HELICOPTER_SINGLE_GPU_ROLLOUT_SHARE", "0").lower() in {"1", "true", "yes", "on"}:
            return
        timing_raw = self.timing_raw
        if self.config.trainer.critic_warmup <= self.global_steps:
