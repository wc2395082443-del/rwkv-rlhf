...
for STEP in 500 1000 1500 2000; do
  ...
  for DS in gsm8k math500; do
    ...
    ODIR="$RUN/full_eval_20260523_1300/${DS}_ckpt${STEP}"
    mkdir -p "$ODIR"
    echo "[$(date '+%F %T')] start $DS ckpt$STEP" | tee -a "$RUN/full_eval_20260523_1300/driver.log"
    ...
    echo "[$(date '+%F %T')] done $DS ckpt$STEP" | tee -a "$RUN/full_eval_20260523_1300/driver.log"
  done
done
