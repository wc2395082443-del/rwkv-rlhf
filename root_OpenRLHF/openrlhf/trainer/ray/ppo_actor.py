...
class ActorPPOTrainer(ABC):
    ...
    def broadcast_to_vllm(self):
        ...
        peft_model = self.actor.model.module
        sync_root = peft_model.base_model.model if hasattr(peft_model, "base_model") else peft_model

        sync_items = []
        for module_name, module in sync_root.named_modules():
            if module_name and isinstance(module, LoraLinear):
                sync_items.append(("lora_module", module_name, module))

        skip_tokens = (".lora_A.", ".lora_B.", ".lora_embedding_", ".lora_magnitude_vector", ".base_layer.")
        for param_name, param in sync_root.named_parameters():
            if any(token in param_name for token in skip_tokens):
                continue
            sync_items.append(("param", param_name, param))

        count, num_params = 0, len(sync_items)

        def _broadcast_param(name, tensor, count, num_params):
            use_ray = getattr(self.strategy.args, "vllm_sync_with_ray", False)
            if torch.distributed.get_rank() == 0:
                refs = [
                    engine.update_weight.remote(name, dtype=tensor.dtype, shape=tensor.shape, empty_cache=count == num_params)
                    for engine in self.vllm_engines
                ]
                ...
                if use_ray:
                    ...
                    collective.broadcast(tensor.data, 0, group_name=self._model_update_group)
                else:
                    self._model_update_group.broadcast(tensor.data, src=0, stream=torch.cuda.current_stream())
                ray.get(refs)

        def _handle_cuda_ipc(name, tensor, count, num_params):
            from torch.multiprocessing.reductions import reduce_tensor

            weight = tensor.data.clone()
        ...
        def _gather_params_ctx(params):
            if not isinstance(params, (list, tuple)):
                params = [params]
            if self.strategy.args.ds_tensor_parallel_size > 1:
                return deepspeed.module_inject.layers.GatherReplacedLayerParams(list(params), peft_model, enabled=True)
            return deepspeed.zero.GatheredParameters(list(params), enabled=self.strategy.args.zero_stage == 3)

        sync_fn = _handle_cuda_ipc if self.use_cuda_ipc else _broadcast_param

        for item_type, name, obj in sync_items:
            count += 1
            if item_type == "param":
                with _gather_params_ctx(obj):
                    sync_fn(name, obj, count, num_params)
                continue

            adapter_names = list(obj.lora_A.keys()) if hasattr(obj, "lora_A") else []
            gather_params = [obj.base_layer.weight]
            for adapter_name in adapter_names:
                gather_params.append(obj.lora_A[adapter_name].weight)
                gather_params.append(obj.lora_B[adapter_name].weight)
            with _gather_params_ctx(gather_params):
                obj.merge(safe_merge=False)
                try:
                    sync_fn(f"{name}.weight", obj.base_layer.weight, count, num_params)
                finally:
                    obj.unmerge()
